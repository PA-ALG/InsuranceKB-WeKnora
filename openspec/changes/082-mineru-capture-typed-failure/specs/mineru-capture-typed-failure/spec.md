...
### Requirement: MTF2 privacy-safe propagation and zero success artifact
...
and content custody failures, and emit no success artifact for any failure. When and
only when an exact raw ZIP was downloaded and the cross-page projection then failed,
capture MAY emit one private failure-custody directory containing the exact raw ZIP
and one metadata file. Both files SHALL be `0600`, the directory SHALL be `0700`,
metadata SHALL use a closed subreason and hashes only, and the pair SHALL NOT satisfy
the success artifact path or any Admission input.
...
#### Scenario: downloaded ZIP fails cross-page projection

- **WHEN** one exact ZIP has been downloaded and a closed cross-page projection check fails
- **THEN** the public error remains `CROSS_PAGE_PROJECTION_INVALID`, no success artifact is
  emitted, and a private raw-ZIP plus closed-reason metadata pair is retained for the
  single authorized diagnostic follow-up
