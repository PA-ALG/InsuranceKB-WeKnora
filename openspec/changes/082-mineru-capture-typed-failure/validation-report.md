...
## 2026-08-06 real-failure diagnostic corrective

- The authorized real terms capture reached provider `done` and downloaded its ZIP,
  but the public terminal was only `CROSS_PAGE_PROJECTION_INVALID`; the downloaded
  ZIP and the exact local projection subreason were then discarded before capture
  returned. Brochure and rate were not called.
- RED 1: a concrete invalid native structural node left no in-memory raw-ZIP/subreason
  custody. RED 2: artifact capture could not publish a private failure pair. RED 3:
  an attacker-controlled reason code was initially admitted by the new publisher.
- GREEN: the concrete capture reader now retains the exact downloaded ZIP and one
  closed subreason only after a cross-page projection/marker failure; capture consumes
  it once and publishes `mineru-raw.zip` plus `mineru-capture-failure.json` under a
  `0700` role directory with both files `0600`. The success artifact remains absent,
  public runner text remains `CROSS_PAGE_PROJECTION_INVALID`, and unrecognized reason
  strings are rejected before directory creation.
- Fresh focused Go regression: affected MinerU converter/capture/runner tests passed
  in both packages. Fresh complete no-provider Python integration: `481 passed`.
- Fresh Ruff, strict mypy (`19 source files`), Go vet and strict OpenSpec validation
  for 082 plus 091–108 passed. Provider/live/DB/PG/WeKnora/Golden/full calls remain
  `0 / NOT RUN`.
