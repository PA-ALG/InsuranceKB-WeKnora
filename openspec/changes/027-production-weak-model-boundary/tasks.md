...
- [ ] T4 写 `PWB2` RED：CLI/API/package exports 的旁路枚举；在入口最小接线
- [ ] T5 写 `PWB3` RED：弱模型失败、模板失配、unknown identity 不得强模型 fallback 或推进候选
- [ ] T6 关闭/隔离历史强 judge/fallback production exports；offline-eval 必须显式 profile
