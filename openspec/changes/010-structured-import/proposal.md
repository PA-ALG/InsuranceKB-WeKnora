...
> 状态：**已条款化（正式 delta 格式），可认领**（2026-07-16 四版：三轮复审后确立冻结 provenance 合同与排期——**T1~T4 即刻；通道二/knowledge 域段（T5 起）排在 018、021 之后**，不插入 018→021→020 关键路径）。
> 依赖：003/007/016/017 已合入 main；**T5 起前置 018（冻结读模型）与 021（排序原语）**。执行者=会话 C3；**Owner 复审=C（新包）+ A（knowledge 域改动全量：tables/models/merge/pages/snapshots/reader + 迁移 0007，按 17 §1）**。
...
> 当前 MVP 子集：见 `mvp-profile.md`；它只交付 known-schema thin，不改变或提前完成下述完整 I1～I9。
...
文件域（四版修正，2026-07-16 三轮复审）：新包（导入模块）+ **knowledge 域改动清单 = `tables.py`/`models.py`/`merge.py`/`pages.py`/`snapshots.py`（FrozenEvidence 变体）与 reader/013 消费的证据冻结合同 + 迁移 0007**——structured 证据必须闭合全消费链（领域模型校验→合并持久化→发布/冻结验证→冻结读取），不是只改表；全部 knowledge 域改动须 **Owner-A 复审**（17 §1）。

**排期（三轮复审裁决，业务方 2026-07-16 确认）**：不插入真实基线关键路径——`018 → 021 → 020` 不变；本 change T1~T4（通道一/登记/映射，不触 knowledge 域）即刻并行，**T5 起基于 021 合入后的 main 接线**（顺带获得排序原语对齐，见 I6）。若未来业务方裁决结构化直入优先于真实基线，须显式改写关键路径并留裁决记录。
