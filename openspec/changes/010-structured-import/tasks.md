...
- [ ] T5 迁移 0007（**基于 021 后 main**）：structured_source_records（id PK/batch_id FK→change_sets/UPDATE+DELETE 双方言触发器拒绝）+ claim_evidence 三值 source_kind（nullable→回填→收紧）+ **ChangeSet 增列与两条 partial unique index** + **read_model_version CHECK (0,1,2)** + 有条件 downgrade（I4/I9）
...
- [ ] T8 通道二导入：双轴幂等 no-op / 同键异 hash collision / **mapping 变化→新 fingerprint 新 ChangeSet + 同值 enrich 不 skip** / 一批 N 记录一个 ChangeSet+重试幂等 / per-source 并发单一有效结果（对齐 021 模式）（I5/I9/I6）
...
1. **Migration**：source_kind 回填正确；三分支 CHECK；源表 UPDATE/DELETE 拒绝；version CHECK(0,1,2)；空数据可 downgrade、有 structured/v2 数据 downgrade fail-closed——SQLite 与 PostgreSQL 都跑（I4）
2. **Identity/幂等**：同键同 hash 同 mapping no-op；同键异 hash collision；同 hash 新 mapping 新 ChangeSet；同值 enrich 新 Evidence 不 skip；一批 N 记录一个 ChangeSet；并发同源仅一个有效结果（I5/I9/I6）
