...
运行清单 SHALL 在开始前冻结恰好一个 ProductVersion exact identity，以及四个
人工确认的 Seed Golden 字段：
...
- **WHEN** 两份冻结材料均不足以支持该 Seed Golden 字段
...
四字段 SHALL 得到 Seed Golden 预期结果，或得到 typed fail-closed 结果；空值、
...
S0-Q SHALL 只在同一两份材料、四个 Seed Golden 字段上完成以下矩阵：
...
satisfied。四字段必须均与 Seed Golden 一致、Evidence 语义支持通过、
