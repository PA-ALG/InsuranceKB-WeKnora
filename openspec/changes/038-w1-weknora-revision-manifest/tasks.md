...
- **事务边界**：(1) attempt 分配与 `parse_status=pending` 写同一事务；
  ...
  翻转 + `current_parse_attempt` fencing 复核）同一事务；(3) tombstone
  来自既有软删事务，W1 只读。
...
```text
无 revision（current_parse_attempt=0 或从未 completed）→ 409
...
| 威胁 | W1 冻结的处理 |
|---|---|
| 走查中并发 reparse（W0 T4，3/3 混版+缺页） | attempt 过滤 + 页后复核当前可服务 attempt；替换后首个页请求 410 `revision_superseded`；0 混版、0 静默缺页（RED-style 竞态合同测试） |
...
| tombstone GC 与读竞态 | W1 不引入硬删 GC；软删行在即 410；任何清理必须遵守配置化保留窗口，窗口内禁止硬删，窗口外 410→404 是唯一退化 |
...
1. manifest 算法纯函数 + 语言中立 vectors（空集、单 chunk、多字节
   UTF-8、非连续 chunk_index、重复 chunk_index typed 拒绝）（unit）；
2. migration/schema 合同：`knowledge_revisions` 列/主键、三个新列及其
   默认值、恰好一对新 versioned migration、历史迁移零修改；
3. attempt 分配：与 pending 写同事务（中断则双无）、严格单调、并发
   reparse 双触发得不同值、trace OpenAttempt 失败不影响；
4. revision 提交：与 completed 翻转同事务（中断则双无）、fencing 复核
   拒绝 stale attempt 提交、failed/cancelled 零 revision 行、同 attempt
   重复提交 typed 失败、行不可变（无 UPDATE 面）；
5. `GET /knowledge/:id/revision` 四态矩阵：200 字段完整性与不可变重
   读、409 三种子况（未曾 completed / in-flight / failed-after-
   committed 含 `last_committed`）、410 tombstone、404 never-existed、
   ACL 矩阵（ro key 可读、无权限 key 既有语义）；
...
7. **W0 T4 竞态复刻（RED-style，先复现混版基线再转 GREEN）**：走查中
   reparse ≥3 次重复，替换后首页请求 410、0 混版、0 静默缺页；变体：
   走查中 DELETE → 410 `knowledge_deleted`（对应 inventory
   「reparse-pagination-delete race rejects mixed revisions」向量）；
...
13. live 复验（受控 lane）：在 023 live 环境重放 W0 探针 p2/p4/p5 的
    W1 版本（绑定端点走查、原子可见性采样、删除区分）；未运行时如实
    报告 NOT RUN。
...
- 逻辑文件 ≤ ~12：migration up/down、`internal/types/`（knowledge.go/
  chunk.go 加列 + revision 类型）、`internal/application/repository/`
  （revision 存取 + chunk attempt 过滤）、
  `internal/application/service/knowledge_create.go`（上传 sha256 +
  分配）、`internal/application/service/knowledge_process.go`（reparse
  分配 + 提交 + fencing）、`internal/handler/`（新只读 revision
  handler）、`internal/router/router.go`（`RegisterKnowledgeRoutes`
  kRead 组两条路由）、`deploy/patches/enterprise-llm-wiki-patch-inventory.yaml`
  （W1 行更新）+ 对应测试文件。

## Tasks

严格 TDD：每任务先落 RED 再实现 GREEN；本 change 为 spec-only，以下
T1–T12 属于未来实现 PR 的执行顺序（开工前先按 HANDOFF/控制板取得实现
窗口授权，并从当时最新 `origin/main` 干净 worktree 开始）。
...
- [ ] T2 RED：schema/migration 合同测试（验收 2）。GREEN：在
  ...
  NOT NULL 约束）。
- [ ] T3 RED：attempt 分配事务/并发/trace 解耦测试（验收 3）。GREEN：
  `knowledge_create.go`/`knowledge_process.go`（含 manual 重建路径）在
  pending 写事务内原子自增并注入任务 payload。
- [ ] T4 RED：revision 提交原子性 + fencing + 失败零行 + 不可变测试
  （验收 4、8）。GREEN：worker 完成路径的提交事务（按 `chunk_index
  ASC` 读回本 attempt text chunk → 算 digest → INSERT revision →
  fencing 复核 → 翻 completed）。
- [ ] T5 RED：`/revision` 四态矩阵 + ACL 测试（验收 5，覆盖
  failed-after-committed 的 `last_committed` body）。GREEN：revision
  handler + `RegisterKnowledgeRoutes` kRead 路由（retrieve capability）
  + tombstone Unscoped 读（先 KB ACL 后 410）。
- [ ] T6 RED：绑定 chunk 读端点语义测试（验收 6）。GREEN：attempt 过滤
  repo 查询 + 分页 clamp 复用 + 每页 revision 绑定块 + 页后复核。
- [ ] T7 RED：**W0 T4 竞态复刻合同测试**——先在无绑定端点语义下复现
  旧新混合/缺页作为 RED 基线，再断言绑定端点 ≥3 次重复 0 混版 0 缺页、
  替换后 410；加 DELETE 变体（验收 7）。GREEN：双检窗口收口，不引入
  锁或快照存储。
- [ ] T8 RED：tombstone 保留窗口与 GC 边界测试（验收 5/13 的窗口子
  项）：窗口内清理不抹除 410、窗口外 404 为唯一退化。GREEN：保留窗口
  配置接线（W1 自身不加 GC）。
