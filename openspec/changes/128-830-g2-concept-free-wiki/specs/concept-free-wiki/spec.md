...
R6 前端权限恢复（2026-09-06）：第一次目录静态构建完成，但执行器私密 umask077 传递到公开 dist，镜像内 nginx 读首页403。保留原镜像及不完整升级回执；用户合理必要扩展授权下，仅将已冻结公开 dist 目录/文件分别规范为0755/0644，证明全部内容字节SHA不变；追加一次 image-only 构建与一次隔离UI替换，不重跑静态编译、不改应用源码/后台/数据库/provider，旧容器保留。运行时健康与浏览器PDF/目录各自验收。
