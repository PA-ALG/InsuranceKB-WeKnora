...
- H1.6 **同类对比缺口**（业务方 2026-07-12 提出）：同险种（进一步同文档族）的产品群里，多数产品该字段为 present 而某产品为 unknown → 疑似缺口（阈值可配，如 ≥60% 同类有值）；输出"该产品大概率有此信息但未抽到/未提供材料"工单，与 015 问答信号形成互补（一个从供给侧对比、一个从需求侧反馈）。
- H1.7 **任务可靠性**（同事 WeKnora 实证反馈 2026-07-16：死信/解析失败对人不可见）：汇总 compiler 死信、judge-queue 长期 unresolved、017 桥接解析失败记录、018 reconciliation 未决工单 → 计数+样本进报告；任一类非零即在报告顶部显式呈现（不得静默沉底），超阈值升级 ReviewItem。
