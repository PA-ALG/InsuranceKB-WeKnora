...
- [ ] T3 漂移三方对账：A=冻结页面 / B=WeKnora adapter 只读回读实际页面 / C=显式版本重编——**独立维度可并存**：单变量夹具无串扰（仅改B/仅改Claim/仅bump digest）+ **B、C 同时变化两维并报（零漏报）** + 规范化 hash（排易变字段）+ 远端不可用 → unknown/degraded（H1.3a）
...
状态：**已条款化，规格复审完成（PR #12 已合入，2026-07-18），按 L4 依赖顺序可认领**。依赖：007/016/018 已合入；H1.5 依赖 009（未落地报 not-applicable，不阻塞其余扫描器）。
