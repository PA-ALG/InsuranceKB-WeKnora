...
4. **E2.2 落点**：凡经变体选择的调用路径＝gapfill（三条返回路径全盖章）；抽取主 prompt 未经变体改动，故不盖章——待变体扩展到抽取路径时同步。
...
状态：**T1~T7 全部完成**，门禁全绿（ruff / mypy 185 files / deterministic 1314 passed）；待 PR 双查（Owner=B 域）。依赖：004/005 已合入。
