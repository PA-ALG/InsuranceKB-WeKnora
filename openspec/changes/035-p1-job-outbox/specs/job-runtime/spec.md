...
**列值同样只收数据**：领域写的列值 SHALL 只接受纯标量（含 `NULL`、布尔、
数值、字符串、时间与由这些构成的 JSON 容器），SHALL 拒绝任何可被编译成 SQL
的表达式对象。否则"收数据不收代码"只在参数名上成立而在参数值上被绕过：子
查询可把 P1 自有表的内容**读出来写进领域表**（跨已声明边界的信息泄漏），
函数调用与字面列可取得环境信息。列名 SHALL 同样按裸标识符校验，并 SHALL 在
反射后确认其在目标表上真实存在——未知列 SHALL 是 typed 输入错误，SHALL NOT
留给编译期抛出原始异常。
...
- **WHEN** 领域写规格的某个列值是 SQL 表达式对象——原始文本片段、函数调用、
  字面列，或读取 P1 自有表的标量子查询
- **THEN** 存储层 SHALL typed 拒绝并回滚完成事务，领域表零行写入；SHALL NOT
  出现自有表内容经领域行被读出的情况。接受侧：纯标量列值（含恰好包含事务
  控制关键字的字符串）SHALL 正常写入
