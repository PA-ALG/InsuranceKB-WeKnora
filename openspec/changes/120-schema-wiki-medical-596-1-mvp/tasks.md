...
- [x] The exact Lane A owner matrix is explicitly extended to the existing
...
- [x] RED a caller-built generic `WikiReleasePreparation`, descriptor-only member, foreign
...
- [x] RED/GREEN persistence in the same `wiki_release_preparations` table: create an
...
- [x] RED/GREEN PostgreSQL JSONB replay: Create derives the manifest/member authority
...
- [x] RED/GREEN exact stored custody: all 75 snapshots must equal the envelope members in
...
- [x] RED/GREEN the Draft control-plane authorization boundary. CreateDraft, exact Draft
...
- [x] RED/GREEN `ReviewDraft`: parse and verify the concrete named-human receipt exactly
...
- [x] RED/GREEN a Ready preparation still cannot activate without a separate exact publish
...
- [x] TDD `GetHeadForWikiKB`, unique scope bootstrap, exact Wiki then derived RAW ACL, and
  distinct Draft/Ready/current/pinned reads with no client-overridable scope or generic
  fallback.
- [x] RED/GREEN the exact Lane C route contract. Bootstrap returns closed
...
- [x] RED/GREEN current entity-version pinning: the exact current endpoint returns only
...
- [x] RED/GREEN scope lifecycle. Active reads derive scope from the sole Head; preparation
...
- [x] RED/GREEN every Draft/Ready/Active member or citation read: follow exact
...
- [x] TDD pure DTO/navigation/citation reducers, no-Active/no-Candidate typed states, exact
  7/67 medical rendering, tri-state fail-closed behavior and no generic fallback.
- [x] Use Vitest with the existing Vue plugin and happy-dom for the production SFC test;
  keep pure TypeScript tests separate.
- [x] Consume only the frozen A/B API and exact revision/page/bbox contract; never use
  current/latest bytes or page 1 as fallback.
- [x] Make product Schema Wiki the default Wiki-enabled knowledge-base surface, keep
...
- [x] Integrate A1 -> B -> A2 -> C without shared-path writes and run only the approved
