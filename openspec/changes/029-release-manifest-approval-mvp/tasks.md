...
- [x] T6 迁移 0013；SQLite focused 与真实 PostgreSQL 16 guard/downgrade/concurrency 矩阵均已通过，PG JUnit `tests=3 skipped=0`，migration 为单 head `0013`
...
- [ ] T9 validation report 已创建且真实 PostgreSQL 已通过；整包独立 review、最终 rebase 后一次 full deterministic、push/PR 与七段时间仍待完成；本会话按总控要求不修改 HANDOFF
