...
_INVENTORY_ID_RE = re.compile(r"[a-z][a-z0-9-]*-v[1-9][0-9]*")
_SQL_IDENTIFIER_RE = re.compile(r"[a-z][a-z0-9_]*")
_DATA_TYPE_RE = re.compile(r"(?:BIGINT|INTEGER|JSONB|TIMESTAMP|VARCHAR\([1-9][0-9]*\))")
...
@dataclass(frozen=True)
class SchemaTableObject:
    semantic_id: str
    object_type: str
    name: str
    columns: tuple[SchemaTableColumn, ...]
...
@dataclass(frozen=True)
class SchemaIndexObject:
    semantic_id: str
    object_type: str
    name: str
    table: str
    unique: bool
    columns: tuple[SchemaIndexColumn, ...]
    predicate: tuple[SchemaPredicate, ...]


type SchemaObject = SchemaColumnObject | SchemaTableObject | SchemaIndexObject


@dataclass(frozen=True)
class EnterpriseSchemaInventory:
    schema_version: int
    inventory_id: str
    owner: str
...
def _parse_table_column(value: object) -> SchemaTableColumn:
    data = _expect_object(value, "table column")
    _expect_closed_keys(data, {"name", "data_type", "nullable"}, "table column")
    data_type = _expect_string(data.get("data_type"), "table column.data_type")
    if _DATA_TYPE_RE.fullmatch(data_type) is None:
        _fail("table column.data_type is not an approved static type")
    return SchemaTableColumn(
        name=_expect_identifier(data.get("name"), "table column.name"),
        data_type=data_type,
        nullable=_expect_bool(data.get("nullable"), "table column.nullable"),
    )


def _parse_primary_key(value: object) -> SchemaPrimaryKey:
    data = _expect_object(value, "primary key")
    _expect_closed_keys(data, {"semantic_id", "columns"}, "primary key")
    return SchemaPrimaryKey(
        semantic_id=_expect_semantic_id(data.get("semantic_id"), "primary key.semantic_id"),
        columns=tuple(
            _expect_identifier(item, "primary key column")
            for item in _expect_list(data.get("columns"), "primary key.columns")
        ),
    )


def _parse_foreign_key(value: object) -> SchemaForeignKey:
    data = _expect_object(value, "foreign key")
    _expect_closed_keys(
        data,
        {
            "semantic_id",
    ...
    columns = tuple(
        _expect_identifier(item, "foreign key column")
        for item in _expect_list(data.get("columns"), "foreign key.columns")
    )
    references_columns = tuple(
        _expect_identifier(item, "foreign key reference column")
        for item in _expect_list(data.get("references_columns"), "foreign key.references_columns")
    )
    if len(columns) != len(references_columns):
        _fail("foreign key local and referenced columns must have equal arity")
    on_delete = _expect_string(data.get("on_delete"), "foreign key.on_delete")
    if on_delete != "CASCADE":
        _fail("foreign key.on_delete must be the proven CASCADE action")
    return SchemaForeignKey(
        semantic_id=_expect_semantic_id(data.get("semantic_id"), "foreign key.semantic_id"),
        columns=columns,
        references_table=_expect_identifier(
            data.get("references_table"), "foreign key.references_table"
        ),
...
def _parse_check(value: object) -> SchemaCheck:
    data = _expect_object(value, "check constraint")
    _expect_closed_keys(data, {"semantic_id", "column", "operator", "value"}, "check constraint")
    operator = _expect_string(data.get("operator"), "check constraint.operator")
    raw_value = data.get("value")
    if operator in {"gt", "gte"}:
        if type(raw_value) is not int:
            _fail("numeric check constraint value must be an integer")
    elif operator == "matches":
        if not isinstance(raw_value, str) or raw_value not in {
            "^[0-9a-f]{64}$",
        }:
            _fail("matches check constraint is not an approved static pattern")
    else:
        _fail("check constraint operator is not approved")
    return SchemaCheck(
        semantic_id=_expect_semantic_id(data.get("semantic_id"), "check constraint.semantic_id"),
        column=_expect_identifier(data.get("column"), "check constraint.column"),
        operator=operator,
        value=raw_value,
    )


def _parse_index_column(value: object) -> SchemaIndexColumn:
    data = _expect_object(value, "index column")
    _expect_closed_keys(data, {"name", "order"}, "index column")
    order = _expect_string(data.get("order"), "index column.order")
    if order not in {"default", "asc", "desc"}:
        _fail("index column.order is not approved")
    return SchemaIndexColumn(
        name=_expect_identifier(data.get("name"), "index column.name"),
        order=order,
    )


def _parse_predicate(value: object) -> SchemaPredicate:
    data = _expect_object(value, "index predicate")
    _expect_closed_keys(data, {"column", "operator", "value"}, "index predicate")
    operator = _expect_string(data.get("operator"), "index predicate.operator")
    raw_value = data.get("value")
    if operator == "is_null":
        if raw_value is not None:
            _fail("is_null predicate value must be null")
    elif operator == "eq":
        if not isinstance(raw_value, str) or not raw_value:
            _fail("eq predicate value must be a non-empty string")
    elif operator == "gt":
        if type(raw_value) is not int:
            _fail("gt predicate value must be an integer")
    else:
        _fail("index predicate operator is not approved")
    return SchemaPredicate(
        column=_expect_identifier(data.get("column"), "index predicate.column"),
        operator=operator,
        value=raw_value,
    )


def _parse_schema_object(value: object) -> SchemaObject:
    data = _expect_object(value, "schema object")
    object_type = _expect_string(data.get("object_type"), "schema object.object_type")
    semantic_id = _expect_semantic_id(data.get("semantic_id"), "schema object.semantic_id")
    if object_type == "column":
        ...
        data_type = _expect_string(data.get("data_type"), "column object.data_type")
        if _DATA_TYPE_RE.fullmatch(data_type) is None:
            _fail("column object.data_type is not an approved static type")
        default = _expect_string(data.get("default"), "column object.default")
        if default not in {"0", "''"}:
            _fail("column object.default is not an approved static literal")
        return SchemaColumnObject(
            semantic_id=semantic_id,
            object_type=object_type,
            table=_expect_identifier(data.get("table"), "column object.table"),
            name=_expect_identifier(data.get("name"), "column object.name"),
            data_type=data_type,
            nullable=_expect_bool(data.get("nullable"), "column object.nullable"),
            default=default,
        )
    if object_type == "table":
        ...
        columns = tuple(
            _parse_table_column(item)
            for item in _expect_list(data.get("columns"), "table object.columns")
        )
        column_names = tuple(column.name for column in columns)
        if len(set(column_names)) != len(column_names):
            _fail("table object columns must be unique")
        primary_key = _parse_primary_key(data.get("primary_key"))
        foreign_keys = tuple(
            _parse_foreign_key(item)
            for item in _expect_list(data.get("foreign_keys"), "table object.foreign_keys")
        )
        checks = tuple(
            _parse_check(item) for item in _expect_list(data.get("checks"), "table object.checks")
        )
        referenced_columns = [
            *primary_key.columns,
            *(column for item in foreign_keys for column in item.columns),
            *(item.column for item in checks),
        ]
        if any(name not in column_names for name in referenced_columns):
            _fail("table constraint references an undeclared column")
        return SchemaTableObject(
            semantic_id=semantic_id,
            object_type=object_type,
            name=_expect_identifier(data.get("name"), "table object.name"),
            columns=columns,
    ...
    if object_type == "index":
        ...
        predicate_value = data.get("predicate")
        if not isinstance(predicate_value, list):
            _fail("index object.predicate must be a list")
        return SchemaIndexObject(
            semantic_id=semantic_id,
            object_type=object_type,
            name=_expect_identifier(data.get("name"), "index object.name"),
            table=_expect_identifier(data.get("table"), "index object.table"),
            unique=_expect_bool(data.get("unique"), "index object.unique"),
            columns=tuple(
                _parse_index_column(item)
                for item in _expect_list(data.get("columns"), "index object.columns")
            ),
            predicate=tuple(_parse_predicate(item) for item in predicate_value),
        )
    _fail("schema object.object_type is not approved")


def load_enterprise_schema_inventory(path: Path) -> EnterpriseSchemaInventory:
    """Load the closed, semantic W1 enterprise schema object inventory."""

    data = _expect_object(_load_yaml(path, "schema inventory"), "schema inventory")
    _expect_closed_keys(
        data,
        {
            "schema_version",
    ...
    if type(data.get("schema_version")) is not int or data["schema_version"] != 1:
        _fail("schema inventory schema_version must be 1")
    inventory_id = _expect_string(data.get("inventory_id"), "schema inventory.inventory_id")
    if _INVENTORY_ID_RE.fullmatch(inventory_id) is None:
        _fail("schema inventory.inventory_id must be a stable versioned ID")
    owner = _expect_string(data.get("owner"), "schema inventory.owner")
    if owner != "W1-weknora-revision-manifest":
        _fail("schema inventory.owner must be the W1 owner")
    authoritative_ref = _expect_string(
        data.get("authoritative_ref"), "schema inventory.authoritative_ref"
    )
    if _REPOSITORY_REF_RE.fullmatch(authoritative_ref) is None or not authoritative_ref.endswith(
        ".md"
    ):
        _fail("schema inventory.authoritative_ref must be a repository spec path")
    objects = tuple(
        _parse_schema_object(item)
        for item in _expect_list(data.get("objects"), "schema inventory.objects")
    )
    semantic_ids: list[str] = []
    signatures: list[tuple[str, str, str]] = []
    for item in objects:
        semantic_ids.append(item.semantic_id)
        if isinstance(item, SchemaColumnObject):
            signatures.append((item.object_type, item.table, item.name))
        elif isinstance(item, SchemaTableObject):
            signatures.append((item.object_type, "", item.name))
            semantic_ids.extend(
                [
                    item.primary_key.semantic_id,
                    *(foreign_key.semantic_id for foreign_key in item.foreign_keys),
                    *(check.semantic_id for check in item.checks),
                ]
            )
        else:
            signatures.append((item.object_type, item.table, item.name))
    if len(set(semantic_ids)) != len(semantic_ids):
        _fail("schema inventory semantic IDs must be unique")
    if len(set(signatures)) != len(signatures):
        _fail("schema inventory objects must be unique")
    return EnterpriseSchemaInventory(
        schema_version=1,
