...
def _prop(version_id: str, predicate: str = "waiting_period", value: str = "90天") -> ProposedClaim:
...
def _merge(session: Session, version_id: str, *, policy: MergePolicy | None = None) -> MergeEngine:
    engine = MergeEngine(session, policy=policy)
    change_set, _ = engine.open_change_set(source_kind="document")
    engine.apply_batch(change_set, [_prop(version_id)])
    return engine
...
def test_k4_1_review_key_stable_and_state_preserved(kb_session: Session) -> None:
    ...
    item, created = ensure_review_item(
        kb_session, review_key=key, type_="low_confidence", subject={"x": 1}
    )
    assert created
    ...
    again, created2 = ensure_review_item(
        kb_session, review_key=key, type_="low_confidence", subject={"x": 2}
    )
...
def test_k4_2_restricted_action_set(kb_session: Session) -> None:
    ...
    with pytest.raises(ValueError, match="受限动作集"):
        resolve_review(kb_session, review.review_key, "escalate", actor="a")

    resolve_review(kb_session, review.review_key, "defer", actor="a")
    ...
    with pytest.raises(ValueError, match="翻案"):
        resolve_review(kb_session, review.review_key, "reject", actor="a")
...
def test_k4_2_reject_keeps_nothing_published(kb_session: Session) -> None:
    ...
    _merge(kb_session, version.id)
    review = kb_session.execute(select(ReviewItem)).scalar_one()
    resolve_review(kb_session, review.review_key, "reject", actor="agent", reason="证据存疑")
    claim = kb_session.execute(select(Claim)).scalar_one()
...
def test_k4_3_only_published_compiles(kb_session: Session) -> None:
    _, version = seed_product(kb_session)
    engine = MergeEngine(kb_session, policy=MergePolicy(auto_apply_add=True))
    change_set, _ = engine.open_change_set(source_kind="document")
    engine.apply_batch(change_set, [_prop(version.id, "waiting_period")])  # published
    engine2 = MergeEngine(kb_session)  # 默认保守 → candidate
    change_set2, _ = engine2.open_change_set(source_kind="document", external_record_id="b2")
    engine2.apply_batch(change_set2, [_prop(version.id, "grace_period", "60天")])

    views = build_page_claims(kb_session, version.id)
...
def test_k4_4_enrich_auto_threshold_configurable(kb_session: Session) -> None:
    ...
    engine.apply_batch(cs1, [_prop(version.id)])
    prop2 = _prop(version.id)
    prop2.evidence[0].knowledge_id = "k-other"
    ...
    engine3 = MergeEngine(kb_session, policy=policy)
    prop3 = _prop(version.id)
    prop3.evidence[0].knowledge_id = "k-third"
    ...
    prop4 = _prop(version.id)
    prop4.confidence = 0.5  # 低于阈值
    prop4.evidence[0].knowledge_id = "k-fourth"
