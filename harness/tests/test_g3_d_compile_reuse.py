...
def test_compile_reuse_binds_actual_results_and_rejects_drift(monkeypatch):
...
def test_review_runtime_reopens_compile_reuse_and_rejects_wrong_prior(monkeypatch):
    ...
    completed = SimpleNamespace(
        request=candidate.request, model_result=candidate.model_compile_result,
        final_result=candidate.compile_result, terminal_sha256='b' * 64,
    )
    ...
    artifacts = {
        ...
        for contract, value in (
            (receipt.contract, receipt), (candidate.request.contract, candidate.request),
    ...
    runtime._validate_g3_prior_stage_results(plan, artifacts)
