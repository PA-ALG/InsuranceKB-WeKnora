...
def test_p2_register_idempotent_and_skip(dataset: Path, session: Session) -> None:
    report1 = register_products(session, dataset)
    assert sorted(report1.created) == ["1820", "1847H"]
    assert len(report1.skipped) == 1 and "坏产品" in report1.skipped[0]

    n_products = session.scalar(select(func.count()).select_from(InsuranceProduct))
    n_docs = session.scalar(select(func.count()).select_from(ProductDocument))
    assert n_products == 2 and n_docs == 2

    # 幂等重跑：零新增
    report2 = register_products(session, dataset)
    assert report2.created == [] and report2.updated == []
    assert sorted(report2.unchanged) == ["1820", "1847H"]
    assert session.scalar(select(func.count()).select_from(InsuranceProduct)) == 2
    assert session.scalar(select(func.count()).select_from(ProductDocument)) == 2


def test_p2_update_changed_fields(dataset: Path, session: Session) -> None:
    register_products(session, dataset)
    meta_path = dataset / "平安福满分（2026）养老年金保险" / "product_meta.txt"
    meta = _meta("1820", "平安福满分（2026）养老年金保险", status="停售")
    meta_path.write_text(json.dumps(meta, ensure_ascii=False))

    report = register_products(session, dataset)
    assert report.updated == ["1820"]
    product = session.execute(
        select(InsuranceProduct).where(InsuranceProduct.product_code == "1820")
    ).scalar_one()
    assert product.status == "停售"


def test_p2_3_aliases_include_registration_no(dataset: Path, session: Session) -> None:
    register_products(session, dataset)
    aliases = {
        row.alias: row.alias_type
        for row in session.execute(select(ProductAlias)).scalars()
    }
