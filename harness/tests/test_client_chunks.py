...
def _chunk(i: int) -> dict[str, object]:
    return {"id": f"c-{i}", "knowledge_id": KID, "content": f"第{i}段", "chunk_index": i}


@respx.mock
async def test_s2_3_paginates_until_short_page(client: WeKnoraClient) -> None:
    ...
    chunks = await client.list_chunks(KID, page_size=2)
...
@respx.mock
async def test_s2_3_empty_knowledge_returns_empty_list(client: WeKnoraClient) -> None:
    respx.get(URL).mock(return_value=httpx.Response(200, json={"data": [], "success": True}))
    assert await client.list_chunks(KID) == []
