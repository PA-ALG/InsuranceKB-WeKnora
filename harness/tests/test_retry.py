...
@respx.mock
async def test_s2_6_retries_5xx_then_succeeds(
...
@respx.mock
async def test_s2_6_gives_up_after_max_attempts(
    client: WeKnoraClient,
    adapter_scope: KnowledgeScope,
) -> None:
    route = respx.get(URL).mock(return_value=httpx.Response(502))
    with pytest.raises(WeKnoraTransientError):
        await client.get_knowledge(adapter_scope, "k-1")
    assert route.call_count == 3


@respx.mock
async def test_s2_6_retries_transport_timeout(
    client: WeKnoraClient,
    adapter_scope: KnowledgeScope,
) -> None:
    route = respx.get(URL).mock(
...
@respx.mock
async def test_s2_6_4xx_no_retry_and_preserves_body(
    client: WeKnoraClient,
    adapter_scope: KnowledgeScope,
) -> None:
    route = respx.get(URL).mock(
