...
@pytest.mark.parametrize(
    ...
    [
        canonical._body,
        canonical._structured,
        resolution._validate_body_text,
        resolution._validate_structured_text,
    ],
)
def test_large_text_control_check_does_not_scan_in_python(
    check: Callable[[str], None], monkeypatch: pytest.MonkeyPatch
) -> None:
    calls = 0

    def counted(value: str) -> int:
        nonlocal calls
        calls += 1
