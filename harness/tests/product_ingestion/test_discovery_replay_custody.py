...
from insurance_harness.product_ingestion.artifact_models import ArtifactOrigin, StageCallSnapshot
...
from insurance_harness.product_ingestion.models import ProductRunSnapshot, ProductRunState
...
def _service(
    *, role: typing.Any, purpose: typing.Any, prompt: typing.Any, new_raw: typing.Any = None
) -> SimpleNamespace:
    class Executor:
...
async def test_generation_reuses_valid_parent_and_retries_only_received_bad_raw(
    ...
    valid_raw = json.dumps({"choices": [{"message": {"content": json.dumps(response)}}]}).encode()
    ...
    output = await run_discovery_generation_stage(
        service=typing.cast(ProductScopeServices, service),
        artifacts=artifacts,
        scope=_scope(),
        run=typing.cast(
            ProductRunSnapshot,
            SimpleNamespace(run_id=child.run_id, retry_of_run_id="parent-run"),
        ),
        stage=products.list_stages(scope=_scope(), run_id=child.run_id)[0],
        job=running,
        request=request,
...
async def test_final_review_reuses_valid_parent_and_retries_only_received_bad_raw(
    ...
    candidate = discovery.project_independent_discovery_response(
        raw=discovery._bytes(_proposal(generation_context)),
        request=request,
        entity_id=entity_id,
        exclusion_index=index,
    ...
    assert (outcome.replayed_call is typing.cast(StageCallSnapshot, parent)) == (
        parent_kind
        in {
            "valid",
            "fenced",
