...
def test_strict_semantic_parser_accepts_only_one_complete_json_fence():
    assert _json(b' \r\n```json\n{"result":1}\n```\n ') == {"result": 1}
...
def test_strict_semantic_parser_rejects_prose_multiple_blocks_and_invalid_json(content):
    with pytest.raises(ValueError):
        _json(content)


def test_field_projection_accepts_single_fenced_json_and_keeps_recorded_raw(source):
