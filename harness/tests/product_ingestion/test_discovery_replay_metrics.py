...
def _settle(artifacts, products, jobs, run, running, stage_key, drafts=()):
    writes = artifacts.prepare_artifact_writes(
        scope=_scope(), run_id=run.run_id, stage_key=stage_key,
    ...
    settlement = products.prepare_stage_settlement(
        scope=_scope(), run_id=run.run_id, stage_id=stage.stage_id,
    ...
    jobs.report_success(
        ...
        domain_writes=writes + settlement.domain_writes, events=settlement.events,
    )


def _record_parent_call(api, factory, *, stage_key, operation, context, prompt, raw, call_id):
    artifacts, products, jobs, parent, running = _start_stage(
        api, factory, run_key="metrics-parent-" + call_id, stage_key=stage_key,
    )
    artifacts.reserve_stage_call(
        scope=_scope(), run_id=parent.run_id, stage_key=stage_key,
        operation_key=operation, dependency_sha256="a" * 64,
        job_id=running.id, generation=running.lease_generation,
        attempt=running.attempt, call_id=call_id, input_sha256=_digest(context),
        model_policy_sha256="b" * 64, prompt_policy_sha256=_digest(prompt),
    )
    request = b'{"provider":"fixture"}'
    artifacts.begin_stage_call(
        scope=_scope(), call_id=call_id, job_id=running.id,
        generation=running.lease_generation,
        request_sha256=_digest(request), request_bytes=request,
    )
    artifacts.record_stage_call_result(
        scope=_scope(), call_id=call_id, job_id=running.id,
        generation=running.lease_generation, request_sha256=_digest(request),
        raw=raw, diagnostic=None, usage={"input_tokens": 7, "output_tokens": 3},
    )
    _settle(artifacts, products, jobs, parent, running, stage_key)
    return parent


def _child(api, factory, *, stage_key, parent_run_id, run_key):
    artifacts, products, jobs, child, running = _start_stage(
        api, factory, run_key=run_key, stage_key=stage_key,
    )
...
def _rule(kind, key, value):
    return artifact(kind, key, json_bytes(value), "a" * 64)


def test_generation_replay_counts_real_parent_once_with_persisted_receipt(api, factory):
    ...
    parent = _record_parent_call(
        api, factory, stage_key="discovery", operation=operation,
        context=content, prompt=discovery.INDEPENDENT_DISCOVERY_PROMPT,
        raw=raw, call_id="metrics-generation-call",
    )
    artifacts, products, jobs, child, running = _child(
        api, factory, stage_key="discovery", parent_run_id=parent.run_id,
...
def test_review_replay_counts_real_parent_with_persisted_proof(api, factory):
    context = {"final_composed_output_hash": "d" * 64}
    content = json_bytes(context)
    raw = json_bytes({"choices": []})
    parent = _record_parent_call(
        api, factory, stage_key="compilation",
        operation="independent-discovery-final-review-" + "d" * 64,
        context=content, prompt=discovery.INDEPENDENT_DISCOVERY_REVIEW_PROMPT,
        raw=raw, call_id="metrics-review-call",
    )
    artifacts, products, jobs, child, running = _child(
        api, factory, stage_key="compilation", parent_run_id=parent.run_id,
...
@pytest.mark.parametrize("tamper", [
    "source_raw_sha256", "source_prompt_sha256", "replayed_from_run_id",
])
def test_generation_replay_rejects_tampered_receipt(api, factory, tamper):
    ...
    parent = _record_parent_call(
        api, factory, stage_key="discovery", operation=operation,
        context=content, prompt=discovery.INDEPENDENT_DISCOVERY_PROMPT,
        raw=raw, call_id="metrics-tamper-call",
    )
    artifacts, products, jobs, child, running = _child(
        api, factory, stage_key="discovery", parent_run_id=parent.run_id,
    ...
    receipt[tamper] = "0" * 64
    _settle(artifacts, products, jobs, child, running, "discovery", (
...
def test_unrelated_rule_artifact_never_counts_as_replay(api, factory):
    artifacts, products, jobs, run, running = _start_stage(
        api, factory, run_key="metrics-unrelated", stage_key="discovery",
    )
    _settle(artifacts, products, jobs, run, running, "discovery", (
