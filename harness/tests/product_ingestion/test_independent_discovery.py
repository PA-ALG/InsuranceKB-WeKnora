...
def test_independent_context_covers_sources_without_field_delta(case):
    request, _delta, entity_id = case
    index = discovery.build_discovery_exclusion_index(request, entity_id)
...
def test_all_original_spans_appear_in_exactly_one_window():
    sources = {}
    for material in ("a", "b"):
        raw = (material + "条件、除外、办理流程。\n") * 450 + material + "尾部说明"
        digest = hashlib.sha256(raw.encode()).hexdigest()
        sources[material] = SourceBlock(
            tenant_id=7, space_id="space", raw_kb_id="raw", knowledge_id=material,
            parse_attempt=1, revision_id="revision-" + material, source_hash=digest,
            parse_hash=digest, parser_identity="parser", block_id="block", page_number=1,
            text=raw, source_type="DOCUMENT",
        )
    windows = route_discovery_source_windows(sources, max_source_chars=2000, max_span_chars=500)
    assert len(windows) > 1
    for ref, source in sources.items():
        ranges = sorted(
            (span["start"], span["end"])
            for window in windows for option in window["source_options"]
            if option["source_ref"] == ref for span in option["spans"]
        )
        assert ranges[0][0] == 0 and ranges[-1][1] == len(source.text)
        assert all(a[1] == b[0] for a, b in zip(ranges, ranges[1:], strict=False))


def test_independent_review_binds_server_final_hash_not_candidate_hash(case):
    request, _delta, entity_id = case
    index = discovery.build_discovery_exclusion_index(request, entity_id)
    with pytest.raises(ValueError, match="final composed output hash mismatch"):
        discovery.render_independent_discovery_review_context(
            ...
            discovery_candidates={
                "definitions": [], "pages": [], "dispositions": [], "sources": []
            },
            final_composed_output_hash="0" * 64,
            final_composed_output=None,
        )


def _proposal(context, *, title="独立办理流程"):
    ...
    return {
        ...
        "proposal": {
            ...
            "transformation": "SYNTHESIZE", "definitions": [], "fields": [],
            "pages": [{
                "page_ref": "new-process", "entity_ref": context["window"]["entity_ref"],
                "stable_key": "independent-process", "title": title, "body": body,
                "evidence": evidence, "concept_refs": [], "conditions": [],
                "exceptions": [], "valid_time": "", "audit_reason": "Useful process",
            }],
        },
        "dispositions": [{
            "candidate_id": "candidate-process", "disposition": "PROPOSED_NEW",
            "text": body, "evidence": evidence, "reason": "Separate process",
            "business_use": "Guide processing", "member_ref": "new-process",
            "existing_target": None,
        }],
    }


def test_schema_field_name_cannot_become_free_page_even_if_unknown(case):
    request, _delta, entity_id = case
    index = discovery.build_discovery_exclusion_index(request, entity_id)
    ...
    with pytest.raises(ValueError, match="schema field duplicate"):
        discovery.project_independent_discovery_response(
            raw=discovery._bytes(proposal), request=request, entity_id=entity_id,
...
def test_valid_independent_page_projects_without_field_delta(case):
    ...
    candidate = discovery.project_independent_discovery_response(
        raw=discovery._bytes(_proposal(context)), request=request, entity_id=entity_id,
        exclusion_index=index, context=context,
    )
...
async def test_generation_stage_records_each_window_without_field_delta(case):
    ...
    executor = Executor()
    settings = SimpleNamespace(templates=[
        SimpleNamespace(role="extract", purpose="g3-independent-discovery",
    ...
    result = await run_discovery_generation_stage(
        service=SimpleNamespace(configuration=SimpleNamespace(model=settings),
                                model_executor=executor),
        artifacts=SimpleNamespace(list_stage_calls=lambda **kwargs: ()), scope=None,
        run=SimpleNamespace(run_id="discovery-run", retry_of_run_id=None),
...
async def test_final_review_binds_actual_composed_hash_and_keeps_whole_group(
    ...
    candidate = discovery.project_independent_discovery_response(
        raw=discovery._bytes(_proposal(context)), request=request,
        entity_id=entity_id, exclusion_index=index, context=context,
    )
    ...
    delta = compiler.record_model_compile(
        request, merged, run_id="independent-final-review",
    ...
    candidates = {
        ...
        "dispositions": [{**_proposal(context)["dispositions"][0],
                          "candidate_id": candidate_id,
                          "member_id": compiler.free_page_id(free_output.pages[0])}],
        "sources": [{"source_ref": context["source_options"][0]["source_ref"],
        ...
        async def execute_stage_call(self, **kwargs):
            ...
            response = {
                ...
                "review": {
                    ...
                    "request_hash": view["request_hash"], "output_hash": view["output_hash"],
    ...
    settings = SimpleNamespace(templates=[SimpleNamespace(
        role="verify", purpose="g3-independent-discovery-review",
        prompt_sha256=hashlib.sha256(discovery.INDEPENDENT_DISCOVERY_REVIEW_PROMPT).hexdigest(),
        max_context_bytes=300_000, template_id="independent-review",
    )])
    outcome = await run_independent_discovery_final_review(
        ...
        run=SimpleNamespace(run_id="discovery-run", retry_of_run_id=None),
        stage=SimpleNamespace(dependency_sha256="b" * 64), job=object(),
        request=request, discovery_candidates=candidates,
