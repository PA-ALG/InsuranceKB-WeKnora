...
from .concept_free_wiki_830_g2 import Evidence, evidence_for, verify_evidence
...
def expand_directory(
    proposal: g.MaterialProposalV1, entry: g.CorpusEntryV1
) -> g.MaterialProposalV1:
    if proposal.material_role != "product-list" or not any(
        "险种代码" in b.text and "险种名称" in b.text for b in entry.blocks
    ):
        return proposal
    blocks = sorted(entry.blocks, key=lambda b: (b.page_number, b.block_id))
    issuers: dict[str, Evidence] = {}
    for block in blocks:
    ...
    if not issuers:
        for entity in proposal.entities:
            for row in proposal.evidence:
                if (
                    entity.issuer
                    and row.purpose == "issuer"
                    and row.entity_proposal_ref == entity.proposal_ref
                    and g._normalized(entity.issuer) in g._normalized(row.evidence.quote)
                ):
    ...
    for block in blocks:
        for match in _ROWS.finditer(block.text):
            ...
            detected_line = detect_product_line(name)
            label = (
                _LINE_LABELS.get(detected_line, "classification_unresolved")
            ...
            entities.append(
                g.EntityProposalV1(
                    ...
                    identity_evidence_ids=tuple(sorted(identity)),
                    labels=(
                        g.LabelProposalV1(
                            taxonomy_label=label,
                            confidence="1.000000",
                            evidence_ids=(ids["classification"],),
                        ),
                    ),
                    primary_label=label,
                    valid_from=None,
    ...
    return proposal.model_copy(
        update={
            "entities": tuple(sorted(entities, key=lambda row: row.proposal_ref)),
            "evidence": tuple(sorted(rows, key=lambda row: row.evidence_id)),
        }
    )


def same_classification(a: g.ClassificationAssignmentV1, b: g.ClassificationAssignmentV1) -> bool:
    return (a.primary_label, a.schema_pack_id, a.schema_version, a.schema_pack_sha256) == (
        b.primary_label,
        b.schema_pack_id,
        b.schema_version,
        b.schema_pack_sha256,
    )


def own_identity_supported(proposal: g.MaterialProposalV1, entity: g.EntityProposalV1) -> bool:
    ...
    return all(
        value is None
        or any(
            row.evidence_id in selected
            and row.purpose == purpose
            and row.entity_proposal_ref == entity.proposal_ref
            and g._normalized(value) in g._normalized(row.evidence.quote)
            for row in proposal.evidence
        )
...
def associate_brochures(
    decisions: tuple[g.MaterialDecisionV1, ...], proposals: g.ProposalBatchV1
    ...
    for parent in decisions:
        ...
        if proposal.material_role != "terms":
            continue
    ...
    for parent in decisions:
        ...
        if (
            ...
            or not set(child.reason_codes) <= allowed
            or entity.issuer is None
            or entity.name is None
            or not own_identity_supported(proposal, entity)
        ):
            result.append(parent)
            continue
        targets = {}
        for target in terms:
            ...
            if (
                a.issuer is None
                or a.name is None
                or a.product_code is None
                or a.version_anchor is None
                or a.version_label is None
            ):
                continue
            if (
                g._normalized(entity.issuer) != a.issuer.normalized_value
                or g._normalized(entity.name) != a.name.normalized_value
                or not same_classification(child.classification, target.classification)
            ):
                continue
            if (
                entity.product_code is not None
                and g._normalized(entity.product_code) != a.product_code.normalized_value
            ):
                continue
            if (
                entity.version_label is not None
                and g._normalized(entity.version_label) != a.version_label.normalized_value
            ):
                continue
            if entity.filing_or_registration is not None and (
                entity.filing_or_registration.kind,
                g._normalized(entity.filing_or_registration.value),
            ) != (a.version_anchor.kind, a.version_anchor.normalized_value):
                continue
            key = (
                target.disposition,
                target.matched_entity_id,
                target.matched_entity_version,
                None
                if target.entity_candidate is None
                else target.entity_candidate.candidate_sha256,
            )
            targets[key] = target
        if len(targets) != 1:
            result.append(parent)
        ...
        values.update(
            ...
            matched_entity_id=target.matched_entity_id,
            matched_entity_version=target.matched_entity_version,
            entity_candidate=target.entity_candidate,
