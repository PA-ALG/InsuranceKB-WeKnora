...
连接串解析顺序：--db-url > HARNESS_DB_URL > sqlite 本地文件（提示测试用）。
...
from insurance_harness.product.classify import DocumentType, classify_document
from insurance_harness.product.register import register_products
from insurance_harness.product.routing import MatchIndex, route_document

_DEFAULT_SQLITE = "sqlite:///harness_product.db"


def _resolve_db_url(arg: str | None) -> str:
    url = arg or os.environ.get("HARNESS_DB_URL")
    if url:
        return url
...
def cmd_classify(args: argparse.Namespace) -> int:
    ...
    print(f"PDF={n} exact命中={exact} ({exact / n:.1%}) LLM调用={llm_used} 未归属={len(unassigned_rows)}")
    return 0
...
def _write_report(path: Path, rows: list[dict[str, object]]) -> None:
    ...
    lines = [
        ...
        f"- 产品 exact 命中：{exact}/{n}（{exact / n:.1%}，门槛 ≥90%）",
    ...
    for r in rows:
        lines.append(
            f"| {r['pdf']} | {r['doc_type']} | {r['type_confidence']} "
