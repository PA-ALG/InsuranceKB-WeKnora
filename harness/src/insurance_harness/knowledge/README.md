...
- `extract-replay` 生成的 Directory revision/page lineage 仅用于 Golden/评估审计，不是数据库导入通道：它没有 attested WeKnora knowledge/raw-KB identity，不能进入生产 source-aware import；又携带 017 audit，不能降格进入 legacy replay。若需回放历史可导入 fixture，必须使用原始无 017 audit 的 legacy pred 并显式 `legacy_replay=True`。
