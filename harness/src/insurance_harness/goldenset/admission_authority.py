    ...
    try:
        ...
        if not destination_removed:
            raise AuthorityPathError(
                "public output rollback is ambiguous"
            ) from exc
...
def write_public_key_descriptor(
    *, descriptor: PublicKeyDescriptor, output_path: Path
) -> None:
...
def _build_envelope(rendered: _RenderedApproval, signature: str) -> VerifiableApprovalEnvelope:
    values = {
...
def sign_rendered_approval(
    *,
    ...
    signature = base64.b64encode(
        private_key.sign(approval_signed_bytes(rendered.domain, rendered.payload))
    ).decode("ascii")
...
def load_public_envelope(path: Path) -> VerifiableApprovalEnvelope:
...
def verify_offline_envelope(
    *,
    ...
    allowed_roles_by_domain: Mapping[ApprovalDomain, frozenset[str]],
    now: datetime,
) -> None:
    ...
    verify_approval_envelope(
        ...
        expected_plan_payload_hash=payload.plan_payload_hash,
