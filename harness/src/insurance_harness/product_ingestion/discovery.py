...
def build_discovery_exclusion_index(request, entity_id: str) -> dict:
    ...
    binding = bindings[0]
    matches = [
        row for row in request.catalog.entries
    ...
    pages = [
        ...
        for row in request.base_request.existing_pages if row.entity_id == entity_id
    ]
    return _json({
...
def render_independent_discovery_contexts(
    *, request, entity_id: str, exclusion_index: dict,
    max_source_chars: int = 24000, max_context_bytes: int = 262144,
) -> tuple[dict, ...]:
...
def independent_discovery_window_audit(request, entity_id: str, context: dict) -> dict:
...
def independent_discovery_window_audits(
    request, entity_id: str, contexts: tuple[dict, ...],
) -> tuple[dict, ...]:
...
def project_independent_discovery_response(
    *, raw: bytes, request, entity_id: str, exclusion_index: dict, context: dict,
    ...
    elif (audit["window_id"] != context["window"]["window_id"]
          or context["coverage"]["audit_sha256"] != _sha(_bytes(audit["coverage"]))):
        raise ValueError("discovery window audit mismatch")
    ...
    for disposition in proposal.dispositions:
        ...
        if disposition.disposition == "PROPOSED_NEW":
            ...
            if disposition.member_ref not in members or disposition.member_ref in proposed:
                raise ValueError("discovery proposed member coverage mismatch")
            member = members[disposition.member_ref]
            if (disposition.existing_target is not None
            ...
            identity_names = (member.title,)
            if hasattr(member, "canonical_key"):
    ...
    aliases = {
        f"s{index + 1}": row["source_ref"]
        for index, row in enumerate(canonical_options)
    }
...
def _comparison_view(request, field_delta, entity_id, binding):
    entries = [
        row
        for row in request.catalog.entries
        if (row.pack.schema_pack_id, row.pack.schema_version, row.pack.schema_pack_sha256)
...
def _discovery_sources(request, binding):
    _, sources, keys = bounded._g3_d_source_index(request)
    current_keys = {
        (block.revision_id, block.block_id)
        for entry in request.resolution_inputs.corpus.entries
...
def _render_compact_window(
    *, request, field_delta, entity_id, binding, entities, comparison, concept_rows,
    source_window, max_source_chars, max_context_bytes,
):
...
def render_discovery_contexts(
    *, request, field_delta: CompileResult, entity_id: str,
    max_source_chars: int = 24000, max_context_bytes: int = 262144,
) -> tuple[dict, ...]:
    ...
    while True:
        ...
        contexts = tuple(
            _render_compact_window(
                request=request, field_delta=field_delta, entity_id=entity_id,
                binding=binding, entities=entities, comparison=comparison,
                concept_rows=concept_rows, source_window=window,
                max_source_chars=budget, max_context_bytes=max_context_bytes,
            )
            for window in windows
        )
...
def discovery_window_audit(request, entity_id: str, context: dict) -> dict:
...
def _exact_generation_context(request, field_delta, context):
    try:
        if context.get("contract") == "product-discovery-context.830.v2":
            expected = next(
                row for row in render_discovery_contexts(
                    request=request, field_delta=field_delta,
...
def _canonical_projection_scope(request, entity_id, context, semantic):
    if context["contract"] != "product-discovery-context.830.v2":
...
def _validate_projection(request, projection, generation):
    if isinstance(projection, DiscoveryGroup):
        reopened = aggregate_discovery_projections(
            request=request,
...
def aggregate_discovery_projections(
    *, request, field_delta: CompileResult,
    projections: tuple[DiscoveryProjection, ...],
) -> DiscoveryGroup:
    ...
    for projection in projections:
        ...
        context = bounded._unique_json_bytes(projection.generation_context)
        ...
        for row in (*projection.new_output.definitions, *projection.new_output.pages):
            identity = (
                row.concept_id
                if isinstance(row, bounded.ConceptDefinition)
                else free_page_id(row)
            )
    ...
    merged = compiler.record_model_compile(
        request, merged_output, run_id=projections[0].merged_delta.execution.run_id,
        implementation="platform-discovery-window-aggregate.830.g3.v1",
        raw=compiler._canonical_json(merged_output),
    )
    return DiscoveryGroup(
        projections, new_output, merged,
        compiler.compose_batch_output(request, merged), field_delta,
...
def render_discovery_review_context(
    *,
    ...
    if isinstance(projection, DiscoveryGroup):
        ...
        generations = tuple(
            _exact_generation_context(request, field_delta, row) for row in context
        )
        if any(_bytes(row) != part.generation_context
    ...
    binding = next(
        row for row in request.entity_bindings
        if row.entity_id == generation["window"]["entity_id"]
    )
    comparison = _comparison_view(
        request, field_delta, generation["window"]["entity_id"], binding
    )
...
def project_discovery_review(
    *,
    ...
    generation = (
        tuple(bounded._unique_json_bytes(row.generation_context)
              for row in projection.projections)
        if isinstance(projection, DiscoveryGroup)
        else bounded._unique_json_bytes(projection.generation_context)
    )
    try:
    ...
    parts = projection.projections if isinstance(projection, DiscoveryGroup) else (projection,)
    expected_ids = {
        row.candidate_id for part in parts for row in part.proposal.dispositions
    }
    if len(ids) != len(set(ids)) or set(ids) != expected_ids:
        raise ValueError("discovery review disposition coverage mismatch")
    reasons = list(review.reasons)
    deferred_update = any(
        row.disposition == "UPDATE_PROPOSAL"
        for part in parts for row in part.proposal.dispositions
    )
    if deferred_update:
...
def render_independent_discovery_review_context(
    *, request, entity_id: str | None, exclusion_index: dict,
    ...
    members = [
        (ConceptDefinition.model_validate(row).concept_id, row)
        for row in definitions
    ] + [
        (free_page_id(FreeWikiPage.model_validate(row)), row)
        for row in pages
    ]
