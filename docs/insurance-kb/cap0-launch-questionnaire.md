...
## 一、Space 与 Source 规模（`space_sources`）

驱动：P2a Space/Source 表和索引合同的行数与隔离预算。
...
## 二、文档形态（`document_shape`）

驱动：chunk/record 表规模与解析、抽取微批（P4b）参数。
...
## 三、SourceRevision 放大率（`revision_amplification`）
...
## 四、EvidenceFragment 上限（`evidence_fragment_limits`）

驱动：行内 inline 上限与外置阈值（033 §5.1）。
...
## 五、Release 与保留窗口（`release_retention`）

驱动：Release/Page/Block 表规模与保留清理策略。
...
## 六、Candidate 与审核（`candidate_review`）

驱动：P6b Candidate 分片、manifest 与审核 SLO 预算。
...
## 七、Active Query（`active_query`）

驱动：P9a SQL 下推分页与返回大小、延迟预算。
...
## 八、Worker 与 provider（`worker_provider`）

驱动：Worker/provider 并发、队列积压与恢复 SLA。
...
## 九、存量回填（`stock_backfill`，2026-07-27 裁决）

驱动：上线初期批量导入的微批与审核吞吐规划（P4b）。
