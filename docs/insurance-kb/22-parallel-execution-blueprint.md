...
| **L4 知识形态** | 010 直入 → 009 概念层 → 012 QA →（011 健康度） | C3 | C（新包）；**010 的 knowledge 域改动（tables/models/merge/pages+迁移 0007）加 A** | 007 ✅；010 的 knowledge 域段前置 018；012 依赖 010 | 010 **通道一/登记映射即刻**，knowledge 域段基于 PR #9 后 main 且**先于 021 开工合入**；009/012 待基础对齐修订 | 导入新包、`concepts/`（新）、QA 模块（新）；迁移 0007/0008/0009；**010 例外：触及 knowledge 域（见其 proposal 影响节）** |
...
main(773f3d1) ──► L2: 008 W1–W3      ┐
             ...
             ──► L5: 024             ┘
PR#9 合并 ─► 010 knowledge 域段（models/merge/pages+0007）─► 合入后 021 才开工（共享域串行）
010 ─► 012；009 ─► 015；020 D4 ─► 024 真实 A/B 验证（零调用侧先行）
```

**串行约束只有两条**：① L0→L1（018→021→020）关键路径；② 010 的 knowledge 域段先于 021 开工合入（共享域串行，2026-07-16 二轮复审确立）。其余全部旁路并行。

## 4. 合并基线政策

1. 规格/文档：以当前 main 为基，不等 PR #9；
2. 实现：不触碰 `knowledge/` → 即刻从 main 开工（008 W1–W3、**010 通道一/登记映射（T1~T4）**、024）；
3. 触碰 knowledge 域/读模型/回滚/发布语义 → PR #9 合入后从新 main 开工（013 实现、008 W4、**010 knowledge 域段（T5 起）**、021——且 010 共享域段先于 021）；
4. 021 的迁移必须排 018 的 `0005` 之后（已预分配 0006）。

## 5. 并行安全护栏（多轨不互踩）

1. **先占号再开工**：change 号与迁移号一律先登记 `openspec/changes/README.md`（022 撞号教训）；
2. **文件域独占**：每轨只写自己的独占目录（上表）；跨域需求以"新文件+最小接线"实现，接线点在 PR 描述中显式列出；**已批准的显式例外 = 010 的 knowledge 域改动**（范围以其 proposal"影响"节清单为准：tables/models/merge/pages+迁移 0007；Owner-A 复审；先于 021）；
...
- **Wave 1（本轮已就绪，可立即认领；四个均为正式 delta 且过 strict 校验）**：008（token→principal+Space 绑定，W4 整页候 018）、010（双通道 Q020 合规；structured 证据全消费链合同，T1~T4 即刻、knowledge 域段候 PR #9 且先于 021）、013（HTTP Streamable 主传输，实现候 PR #9）、024（机制/版本化/护栏/非退化框架，真实召回归 020 D4）；
