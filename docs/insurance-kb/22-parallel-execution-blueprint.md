...
| **L4 知识形态** | 010 直入 → 009 概念层 → 012 QA →（011 健康度） | C3 | C（新包）；**010 的 claim_evidence DDL 加 A** | 007 ✅；012 依赖 010 | 010 **即刻**；009/012 待基础对齐修订 | 导入新包、`concepts/`（新）、QA 模块（新）；迁移 0007/0008/0009 |
...
- **Wave 1（本轮已就绪，可立即认领；四个均为正式 delta 且过 strict 校验）**：008（token→Space 绑定，W4 整页候 018）、010（双通道：meta bootstrap 零 Claim / 可信业务源→Claim，Q020 合规）、013（HTTP Streamable 主传输，实现候 PR #9）、024（机制/版本化/护栏/非退化框架，真实召回归 020 D4）；
