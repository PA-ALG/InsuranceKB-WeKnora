...
## 何时用（先分风险，避免把每个 PR 都做成 019）

- **A 级必须完整使用**：权限、迁移、审批、发布、租户隔离、hash identity、准入和自动 merge 路径；
- **B 级使用规格/不变量/定向红队部分**：模板、抽取、路由、融合、冲突，并以 focused + Golden Slice 为主要证据；
- **C 级不跑通用红队**：UI、文档、只读接线，只需契约测试/smoke 和诚实边界报告。

风险级在任务卡中由总体规划窗口确定，执行会话不得自行把 A 降为 B/C，也不得把 C 级工作升级成多轮全量 gauntlet。
...
3. **按风险自派定向红队**（见下“红队配方”）：A 级必须覆盖对应高风险面；B 级只攻本 PR 的核心不变量；C 级默认不跑。需要红队时写脚本 live 复现才算数，不接受走查猜测。
4. **每条发现：live 复现 → TDD 锁 → 修 → 复跑**：先复现（数值/结论与报告对齐）**再**动手；测试名引用规格条款号（如 `test_q4_6_...`）；修完复跑证明关闭。**别假设发现是真的或已修的。**
5. **分层门禁**：每个 RED/GREEN 只跑精确测试，目标 ≤90 秒；任务收口跑领域套件，目标 ≤3 分钟。只有达到 PR ready 时，本地运行一次完整 deterministic；CI 再独立运行一次：
   ```bash
   cd harness && uv run ruff check . && uv run mypy src tests && uv run pytest -m "not live and not integration_postgres" -q
   ```
   不在每个小步骤或每轮 review 重复完整 2700+ 套件。绿了再对着下面“问题清单”中与本 PR 风险相关的条目自查。
...
| **规格不红队就盖章**（PR #11 教训，2026-07-16） | 给存量 spec 补齐条款后就标“可认领”，没核对事实性主张、外部系统真实能力和测量机制的证明力边界 | spec 的错误比代码放大倍数高：盖“可认领”章前核对每个可证伪断言、NS-RIGHTS/NS-0/适用 admission 状态与测量证明力；第一方迁移要有 provenance，第三方许可证要单列 |

## 红队配方（可直接照抄）

只有 A/B 级需要时，送复审前才派定向 Agent，每支只攻一个面；数量由风险面决定，不以“越多越安全”为目标：
...
Reviewer 首轮必须一次性提交完整 finding，第二轮只验证关闭；出现第三轮时停止逐条补丁，由总体规划窗口裁决接口重设计、拆 PR 或驳回 finding。
