...
| **规格不红队就盖章**（PR #11 教训，2026-07-16） | 给存量 spec 补齐条款后就标"可认领"，没回源核对其**事实性主张**——仓库既有裁决（如 Q020 元数据≠证据）、外部系统真实能力（如 WeKnora 禁用 stdio）、测量机制的证明力边界（replay key=prompt 哈希 ⇒ 回放证不了 prompt 提升） | spec 的错误比代码放大倍数高（同时带偏多个并行实现分支）：盖"可认领"章前，逐条核对 spec 里每个可证伪断言的出处；触发条件只允许运行时可得信息；验收指标先问"这个测量方法在原理上能证明这个结论吗" |
