...
同轮必须显式修正validate_delta_output的per-owner派生allowlist：该实体selected materials全部blocks，加该实体actual base fields/pages的Evidence来源；existing concept definitions只沿这些成员已验证的concept关联加入其Evidence来源，不把全空间或另一实体的carry来源自动授予该owner。此派生仍只读现有base/refs，不增加可写声明。新增合法owner carry Evidence用于delta页面的正向测试，以及另一owner-only carry来源借用的拒绝测试；同时保留selected正文块、缺块、额外块和异文拒绝。
