...
1. `preflight --output <new-private-json>` is read-only with respect to Docker,
   PostgreSQL, Redis and the app.  It verifies the five frozen evidence files,
   ...
   Target absence requires both Docker's nonzero inspect status and JSON `[]`,
   plus a kind-specific exact-name missing-object error.  For the network this
   is the observed daemon text `network weknora-g3-830-internal not found`;
   daemon, transport and permission failures are rejected rather than treated
   as absence.
2. `apply --preflight ... --authorization ... --private-auth ... --receipt
   ...
   failure.  It performs no automatic rollback, resume or retry.  A failed run
   stops and reinspects each target container and records `STOP_INCOMPLETE` if
   any stop, inspection, or staged-secret cleanup cannot be proved complete.
   All partial containers, volumes, DB state and the receipt remain for
   inspection.
...
- Use a fresh local human login from a private, regular, non-link mode-0600 file
  containing exactly `email` and `password`.  Build the KB PUT from the full GET fields and change
...
The review-repair draft additionally binds the frozen existing-model evidence,
the source app's exact DB host/port/network alias to the inspected PostgreSQL
container image, and target `current_database`/`current_user`.  Human login,
active tenant 10003, Admin-or-Owner membership, and `/auth/me` identity are
verified before either PUT.  Each GET-derived DTO is checkpointed before its
single PUT and receives an immediate full non-dynamic expected-after readback;
the model permits only the `base_url` change and preserves custom headers.

Source resources must be regular non-link files.  Their outbound tar must have
exactly the four expected members with matching sizes and SHA values before
extraction.  DocReader variables use an explicit reviewed key set, while every
env key/value rejects CR/LF/NUL.  Private env files are removed immediately
after stopped-container creation and are also cleaned on exceptions.  Failure
stops and reinspects SEED/app/DocReader/Redis one by one; any unresolved running
container or inspection/cleanup failure records `STOP_INCOMPLETE`.
