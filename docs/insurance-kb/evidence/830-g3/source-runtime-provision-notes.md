...
1. `preflight --output <new-private-json>` is read-only with respect to Docker,
   ...
   Target absence requires both Docker's nonzero inspect status and the
   kind-specific `No such ...` error with JSON `[]`; daemon, transport and
   permission failures are rejected rather than treated as absence.
