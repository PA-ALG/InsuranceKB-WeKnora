# G3 C model semantic response and local assembler — design candidate 02
...
3. Treat `start/end` as code-point offsets. Require `0 <= start < end <= len(block.text)`,
   ...
   The exact offset disambiguates repeated text inside W1, so block-wide quote uniqueness is
   neither required nor inferred. Reopen the frozen native page named by the pre-model page
   map and require exactly one occurrence of the quote on that canonical native-page
   projection. Resolve that occurrence against the frozen character boxes: every visible
   Unicode code point in the quote must have a complete corresponding box, in order, with
   finite positive dimensions wholly inside that selected page's declared bounds. Whitespace
   may follow the capture's explicit whitespace mapping, but no visible code point may be
   skipped or borrowed from another page. The earlier page-map proof anchor does not prove
   this new quote range. A missing/duplicate native-page occurrence, incomplete box coverage,
   out-of-page box, or page mismatch is rejected; no trimming, normalization, fuzzy search,
   page switching or wider-span repair is allowed.
...
## Repeated-quote boundary examples

These examples are synthetic and define only the assembler check.

- **Positive:** W1 block text is `责任A；责任A`; the model selects `start=4,end=7`, whose exact
  slice is the second `责任A`. The frozen page map selects page 2, page 2's canonical native
  projection contains `责任A` exactly once, and each of `责`、`任`、`A` has an in-bounds box.
  Accept it. The identical quote at W1 offsets `0:3` does not make the selected offset
  ambiguous and does not invalidate the evidence.
- **Negative:** the same W1 offset is selected, but the selected page's native projection is
  `责任A；责任A`. Reject it because the native occurrence is not unique, even if both occurrence
  box sequences are otherwise complete. Also reject a single native occurrence if any visible
  code point lacks a box or any box crosses the selected page boundary.
