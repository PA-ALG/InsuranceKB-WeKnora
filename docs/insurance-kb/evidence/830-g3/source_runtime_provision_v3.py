...
FROZEN_FILES = {
    ...
    "existing-model-config-readonly.json":
        "1748e661fcad996581e2c5a73f61708c9fa9d3e5805f448c1f38dac21dceda15",
}
...
REMOTE_APP_ENV = "/run/g3-830-private/app.env"
REMOTE_REDIS_ENV = "/run/g3-830-private/redis.env"

SOURCE_FILES_VOLUME = "weknora-g2-594-files"

DOCREADER_ENV_ALLOWLIST = frozenset({
    "DOCREADER_GRPC_MAX_WORKERS", "DOCREADER_GRPC_MAX_FILE_SIZE_MB", "DOCREADER_GRPC_PORT",
    "DOCREADER_DOCX_MAX_PAGES", "DOCREADER_MARKITDOWN_MAX_WORKERS",
    "DOCREADER_ODL_MAX_WORKERS", "DOCREADER_ODL_HYBRID",
    "DOCREADER_ODL_HYBRID_MODE", "DOCREADER_ODL_HYBRID_FALLBACK",
    "DOCREADER_ODL_MARKDOWN_WITH_HTML", "DOCREADER_IMAGE_OUTPUT_DIR",
    "DOCREADER_PDF_RENDER_MAX_WORKERS", "DOCREADER_PDF_RENDER_PARALLELISM",
    "DOCREADER_PDF_RENDER_DPI", "DOCREADER_PDF_JPEG_QUALITY", "DOCREADER_PDF_RENDER_MAX_EDGE",
    "DOCREADER_PDF_FORCE_SCANNED", "DOCREADER_PDF_WORD_GAP_WIDTH_RATIO",
    "DOCREADER_PDF_MARGIN_COL_WIDTH_RATIO", "DOCREADER_PDF_MIN_HEADING_LINE_CHARS",
    "DOCREADER_PDF_SANITIZE_TEXT", "DOCREADER_PDF_STRIP_CHART_DEBRIS",
    "DOCREADER_PDF_RENDER_VECTOR_FIGURES", "DOCREADER_PDF_DETECT_HEADINGS",
    "DOCREADER_PDF_LAYOUT_ORDERING", "DOCREADER_PDF_FILTER_HIDDEN_TEXT",
    "DOCREADER_PDF_SCAN_IMAGE_RATIO", "DOCREADER_PDF_SCAN_MIN_CHARS",
    "DOCREADER_PDF_EXTRACT_EMBEDDED_IMAGES", "DOCREADER_PDF_EMBED_MIN_PIXELS",
    "DOCREADER_PDF_EMBED_MIN_AREA_RATIO", "DOCREADER_PDF_EMBED_REPEAT_PAGE_FRAC",
    "DOCREADER_PDF_EMBED_MAX_IMAGES", "DOCREADER_PDF_MIN_CHART_REGION_AREA",
    "DOCREADER_PDF_MIN_CHART_REGION_CHARS", "DOCREADER_PDF_MAX_CHART_REGION_AREA",
    "DOCREADER_PDF_MAX_FIGURE_HEIGHT_RATIO",
})
DOCREADER_ENV_FORCED_EMPTY = frozenset({
    "DOCREADER_EXTERNAL_HTTP_PROXY", "DOCREADER_EXTERNAL_HTTPS_PROXY", "DOCREADER_ODL_HYBRID_URL",
})
KB_DYNAMIC_RESPONSE_KEYS = frozenset({
    "updated_at", "knowledge_count", "chunk_count", "processing_count", "share_count", "is_processing",
})
...
class Runner:
    ...
    def _docker_process(self, *args: str, data: bytes | None = None) -> subprocess.CompletedProcess:
        return subprocess.run(self.prefix + list(args), input=data, stdout=subprocess.PIPE,
    ...
    def docker(self, *args: str, data: bytes | None = None, allow_failure: bool = False) -> bytes:
        ...
        proc = self._docker_process(*args, data=data)
    ...
    def inspect(self, kind: str, name: str, *, allow_missing: bool = False) -> dict | None:
        proc = self._docker_process(kind, "inspect", name)
        if proc.returncode:
            if allow_missing:
                try:
                ...
                missing_fragments = {
                    "container": (f"No such container: {name}", f"No such object: {name}"),
                    # Exact Docker daemon text observed by root on the live
                    # Colima transport; the trailing Colima fatal line is not
                    # part of the absence decision.
                    "network": (f"network {name} not found",),
                    "volume": (f"No such volume: {name}", f"get {name}: no such volume"),
                }.get(kind, ())
                if value == [] and any(fragment in stderr for fragment in missing_fragments):
                    return None
            # Transport, daemon and permission failures cannot satisfy an
            # absence gate, even if a client happened to emit JSON [].
            raise RuntimeError(f"docker inspect failed for {kind}/{name}, rc={proc.returncode}")
        value = json.loads(proc.stdout)
        if len(value) != 1:
...
def validate_env_mapping(values: dict[str, str]) -> None:
    for key, value in values.items():
        if not re.fullmatch(r"[A-Z][A-Z0-9_]*", key):
            raise RuntimeError("invalid runtime environment key")
        if not isinstance(value, str) or any(c in value for c in ("\r", "\n", "\x00")):
            raise RuntimeError(f"invalid runtime environment value for {key}")
...
def tar_manifest(blob: bytes, *, select: set[str] | None = None) -> dict[str, dict]:
    ...
    with tarfile.open(fileobj=io.BytesIO(blob), mode="r:*") as tf:
        for member in tf.getmembers():
            ...
            if name in result:
                raise RuntimeError("tar contains a duplicate regular-file member")
...
def database_identity(r: Runner, database: str) -> dict:
    return json.loads(r.psql(database,
        "SELECT json_build_object('database',current_database(),'user',current_user)", readonly=True))


def source_pg_binding(source_app: dict, pg: dict, app_env: dict[str, str]) -> dict:
    host, port = app_env.get("DB_HOST", ""), app_env.get("DB_PORT", "")
    database, user = app_env.get("DB_NAME", ""), app_env.get("DB_USER", "")
    if not host or port != "5432" or database != SOURCE_DB or user != DB_USER:
        raise RuntimeError("source database identity drift")
    app_networks = source_app.get("NetworkSettings", {}).get("Networks", {})
    pg_networks = pg.get("NetworkSettings", {}).get("Networks", {})
    matches = []
    for network in sorted(set(app_networks).intersection(pg_networks)):
        aliases = pg_networks[network].get("Aliases") or []
        if host in aliases:
            matches.append(network)
    if len(matches) != 1:
        raise RuntimeError("source DB_HOST does not map uniquely to the exact PostgreSQL container")
    return {"host": host, "port": port, "database": database, "user": user,
            "network": matches[0],
            "container": PG_CONTAINER, "image": pg.get("Image")}


def validate_apply_source_environment(values: dict[str, str], preflight_source: dict) -> None:
    if not isinstance(values, dict) or values.get("DB_NAME") != SOURCE_DB \
            or values.get("DB_USER") != DB_USER \
            or preflight_source.get("source_db") != SOURCE_DB \
            or preflight_source.get("db_user") != DB_USER:
        raise RuntimeError("source database identity drift since preflight")
    binding = preflight_source.get("postgres_binding")
    if not isinstance(binding, dict) or binding.get("database") != SOURCE_DB \
            or binding.get("user") != DB_USER:
        raise RuntimeError("preflight PostgreSQL binding lacks database identity")


def validate_apply_parser_environment(values: dict[str, str]) -> None:
    if not isinstance(values, dict):
        raise RuntimeError("preflight parser environment is not an object")
    permitted = DOCREADER_ENV_ALLOWLIST | DOCREADER_ENV_FORCED_EMPTY
    if not set(values).issubset(permitted):
        raise RuntimeError("preflight parser environment contains an unreviewed key")
    forced = {
    ...
    if any(values.get(key) != expected for key, expected in forced.items()):
        raise RuntimeError("preflight parser environment forced value drift")
    validate_env_mapping(values)


def build_parser_environment(doc_env: dict[str, str]) -> dict[str, str]:
    unknown = {key for key in doc_env if key.startswith("DOCREADER_")
               and key not in DOCREADER_ENV_ALLOWLIST
               and key not in DOCREADER_ENV_FORCED_EMPTY}
    if unknown:
        raise RuntimeError("source DocReader has an unreviewed environment key")
    result = {key: value for key, value in doc_env.items() if key in DOCREADER_ENV_ALLOWLIST}
    result.update({
        "DOCREADER_EXTERNAL_HTTP_PROXY": "",
        "DOCREADER_EXTERNAL_HTTPS_PROXY": "",
        "DOCREADER_ODL_HYBRID_URL": "",
    ...
    validate_apply_parser_environment(result)
    return result
...
def preflight(args: argparse.Namespace) -> None:
    ...
    if app_env.get("DB_NAME") != SOURCE_DB or app_env.get("DB_USER") != DB_USER:
        raise RuntimeError("source database identity drift")
    pg_binding = source_pg_binding(source_app, pg, app_env)
    if database_identity(r, SOURCE_DB) != {"database": SOURCE_DB, "user": DB_USER}:
        raise RuntimeError("source database connection identity drift")
    ...
    for rel, expected_size, expected_sha in EXPECTED_FILES:
        r.docker("exec", SOURCE_APP, "test", "-f", f"/data/files/{rel}")
        r.docker("exec", SOURCE_APP, "test", "!", "-L", f"/data/files/{rel}")
    ...
    parser_env = build_parser_environment(env_map(source_doc))

    safe_source = {
        "app_image": source_app["Image"],
        "docreader_image": source_doc["Image"],
        "postgres_image": pg["Image"],
        "postgres_binding": pg_binding,
        "source_db": SOURCE_DB,
        "db_user": DB_USER,
...
class ApplyState:
    ...
    def stop(self, message: str, stop_evidence: dict, cleanup_evidence: dict) -> None:
        incomplete = bool(stop_evidence.get("failures")) or not cleanup_evidence.get("clean", False)
        self.value["status"] = "STOP_INCOMPLETE" if incomplete else "STOPPED"
        self.value["stopped_at"] = utcnow()
        self.value["failure"] = message
        self.value["stop_evidence"] = stop_evidence
        self.value["secret_cleanup"] = cleanup_evidence
        try:
            atomic_json(self.path, self.value)
        except Exception:
            pass
...
def stop_and_inspect_targets(r: Runner) -> dict:
    results, failures = {}, []
    for name in (SEED, APP, DOCREADER, REDIS):
        try:
            before = r.inspect("container", name, allow_missing=True)
            if before is None:
                results[name] = {"before": "absent", "after": "absent"}
                continue
            was_running = bool(before.get("State", {}).get("Running"))
            stop_rc = None
            if was_running:
                stop_rc = r._docker_process("stop", "--time", "3", name).returncode
            after = r.inspect("container", name, allow_missing=True)
            still_running = bool(after and after.get("State", {}).get("Running"))
            results[name] = {"before_running": was_running, "stop_rc": stop_rc,
                             "after_exists": after is not None, "after_running": still_running}
            if still_running or (was_running and stop_rc != 0):
                failures.append(name)
        except Exception:
            results[name] = {"inspection": "failed"}
            failures.append(name)
    return {"containers": results, "failures": failures}
...
def cleanup_remote_secret_files(r: Runner) -> dict:
    if r.prefix[0] == "docker":
        return {"attempted": False, "clean": True, "reason": "native-transport-never-stages"}
    cmd = ["colima", "ssh", "--profile", "default", "--", "sudo", "/bin/sh", "-ceu",
           'rm -f "$1" "$2"; test ! -e "$1"; test ! -e "$2"', "g3-cleanup",
           REMOTE_APP_ENV, REMOTE_REDIS_ENV]
    try:
        proc = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)
    except Exception:
        # Keep exception text out of the receipt because process-launch errors
        # can contain command context.  The caller must persist clean=false.
        return {"attempted": True, "clean": False, "execution": "failed"}
    return {"attempted": True, "clean": proc.returncode == 0, "rc": proc.returncode}


def copy_tar(r: Runner, source_container: str, source_root: str,
             expected: dict[str, dict], target_container: str, target_root: str) -> None:
    relative = sorted(expected)
    for name in relative:
        validate_relative(name)
    blob = r.docker("exec", source_container, "tar", "-C", source_root, "-cf", "-", *relative)
    if tar_manifest(blob) != dict(sorted(expected.items())):
        raise RuntimeError("copy tar source manifest mismatch")
...
def verify_seed_file(r: Runner, root: str, rel: str, size: int, digest: str) -> None:
    r.docker("exec", SEED, "test", "-f", f"{root}/{rel}")
    r.docker("exec", SEED, "test", "!", "-L", f"{root}/{rel}")
...
def _without_top_keys(value: dict, keys: frozenset[str]) -> dict:
    return {k: v for k, v in value.items() if k not in keys}


def _normalized_kb_readback(value: dict) -> dict:
    result = json.loads(json.dumps(value))
    chunking = result.get("chunking_config")
    if not isinstance(chunking, dict):
        raise RuntimeError("KB readback chunking_config is not an object")
    chunking.setdefault("token_limit", 0)
    chunking.setdefault("languages", [])
    return _without_top_keys(result, KB_DYNAMIC_RESPONSE_KEYS)


def _derive_kb_capabilities(kb: dict, indexing: dict) -> dict:
    return {
        "vector": bool(indexing.get("vector_enabled")),
        "keyword": bool(indexing.get("keyword_enabled")),
        "wiki": bool(indexing.get("wiki_enabled")),
        "graph": bool(indexing.get("graph_enabled")),
        "faq": kb.get("type") == "faq",
    }


def _frozen_kb_selected(frozen: dict[str, object]) -> dict:
    cfg = frozen["runtime-config-readonly.json"]
    rows = [x["selected"] for x in cfg["observations"]
            if x["path"] == f"/api/v1/knowledge-bases/{RAW_KB_ID}"]
    if len(rows) != 1:
        raise RuntimeError("frozen KB identity is not unique")
    return rows[0]


def configure_api(auth_path: Path, checkpoint) -> dict:
    if not auth_path.is_file() or auth_path.is_symlink():
        raise RuntimeError("private auth path must be a regular non-link file")
    ...
    active_tenant = login.get("active_tenant")
    memberships = login.get("memberships")
    if not isinstance(login_user, dict) or not login_user.get("id") \
            or login_user.get("email") != auth["email"] or login_user.get("is_active") is not True:
        raise RuntimeError("login user identity mismatch")
    if not isinstance(active_tenant, dict) or active_tenant.get("id") != TENANT_ID:
        raise RuntimeError("login active tenant is not the frozen tenant")
    roles = [m.get("role") for m in memberships or []
             if isinstance(m, dict) and m.get("tenant_id") == TENANT_ID]
    if len(roles) != 1 or roles[0] not in {"owner", "admin"}:
        raise RuntimeError("login session is not a human Admin for the frozen tenant")
    me = unwrap_data(api_request("GET", "/api/v1/auth/me", token=token))
    me_roles = [m.get("role") for m in me.get("memberships", [])
                if isinstance(m, dict) and m.get("tenant_id") == TENANT_ID]
    if not isinstance(me.get("user"), dict) or me["user"].get("id") != login_user["id"] \
            or me["user"].get("is_active") is not True \
            or not isinstance(me.get("tenant"), dict) or me["tenant"].get("id") != TENANT_ID \
            or me_roles != roles:
        raise RuntimeError("authenticated user/tenant/role readback mismatch")
    checkpoint("validated-human-admin-session", {
        "user_id": login_user["id"], "tenant_id": TENANT_ID, "role": roles[0]})

    frozen = require_frozen_evidence()
    frozen_kb = _frozen_kb_selected(frozen)
    kb_before = unwrap_data(api_request(
        "GET", f"/api/v1/knowledge-bases/{RAW_KB_ID}", token=token))
    expected_kb_identity = {
        "id": frozen_kb["id"], "tenant_id": frozen_kb["tenant_id"],
        "embedding_model_id": frozen_kb["embedding_model_id"],
        "storage_backend_id": frozen_kb["storage_backend_id"],
        "vector_store_id": frozen_kb["vector_store_id"],
    }
    if any(kb_before.get(k) != v for k, v in expected_kb_identity.items()) \
            or kb_before.get("summary_model_id") is not None \
            or kb_before.get("question_generation_config") != {"enabled": False, "question_count": 0} \
            or kb_before.get("chunking_config") != frozen_kb["chunking_config"] \
            or kb_before.get("indexing_strategy") != frozen_kb["indexing_strategy"]:
        raise RuntimeError("KB GET identity drift")
    required_kb = ["name", "description", "type", "capabilities", "chunking_config",
                   "image_processing_config", "faq_config", "wiki_config", "indexing_strategy"]
    if any(key not in kb_before for key in required_kb):
        raise RuntimeError("KB GET lacks a full update field")
    if not isinstance(kb_before["name"], str) or not isinstance(kb_before["description"], str) \
            or not isinstance(kb_before["chunking_config"], dict) \
            or not isinstance(kb_before["image_processing_config"], dict) \
            or not isinstance(kb_before["indexing_strategy"], dict):
        raise RuntimeError("KB GET update DTO type mismatch")
    ...
    expected_kb_after = json.loads(json.dumps(kb_before))
    expected_kb_after["chunking_config"] = chunking
    expected_kb_after["indexing_strategy"] = indexing
    expected_kb_after["capabilities"] = _derive_kb_capabilities(kb_before, indexing)
    checkpoint("kb-api-prewrite-validated", {
        "before_sha256": sha256(canonical_bytes(kb_before)),
        "payload_sha256": sha256(canonical_bytes(kb_payload))})
    kb_put = api_request("PUT", f"/api/v1/knowledge-bases/{RAW_KB_ID}", token=token, body=kb_payload)
    checkpoint("kb-api-put-succeeded", {"response_sha256": sha256(canonical_bytes(kb_put))})
    kb_after = unwrap_data(api_request("GET", f"/api/v1/knowledge-bases/{RAW_KB_ID}", token=token))
    if _normalized_kb_readback(kb_after) != _normalized_kb_readback(expected_kb_after):
        raise RuntimeError("KB API full readback mismatch")
    checkpoint("kb-api-postwrite-readback", {
        "after_sha256": sha256(canonical_bytes(kb_after)), "full_expected_after": True})
    ...
    required_model = {"id", "tenant_id", "name", "display_name", "description", "source", "type",
                      "is_builtin", "parameters"}
    required_params = {"base_url", "interface_type", "embedding_parameters", "parameter_size",
                       "provider", "supports_vision"}
    if not required_model.issubset(model_before) or not isinstance(params, dict) \
    ...
    model_evidence = frozen["existing-model-config-readonly.json"]
    parsed_url = urlsplit(params["base_url"])
    if model_before.get("name") != model_evidence["embedding_model"]["name"] \
            or model_before.get("type") != model_evidence["embedding_model"]["type"] \
            or model_before.get("source") != model_evidence["embedding_model"]["source"] \
            or params.get("embedding_parameters") != model_evidence["embedding_parameters"] \
            or params.get("provider") != model_evidence["provider"] \
            or params.get("interface_type") != model_evidence["interface_type"] \
            or sorted((params.get("custom_headers") or {}).keys()) != model_evidence["custom_header_names"] \
            or {"scheme": parsed_url.scheme, "host": parsed_url.hostname, "port": parsed_url.port,
                "path": parsed_url.path} != model_evidence["base_url_sanitized"]:
        raise RuntimeError("embedding model frozen identity/config drift")
    ...
    expected_model_after = json.loads(json.dumps(model_before))
    expected_model_after["parameters"]["base_url"] = GUARD_BASE_URL
    checkpoint("model-api-prewrite-validated", {
        "before_sha256": sha256(canonical_bytes(model_before)),
        "payload_sha256": sha256(canonical_bytes(model_payload))})
    model_put = api_request("PUT", f"/api/v1/models/{EMBEDDING_MODEL_ID}", token=token, body=model_payload)
    checkpoint("model-api-put-succeeded", {"response_sha256": sha256(canonical_bytes(model_put))})
    model_after = unwrap_data(api_request("GET", f"/api/v1/models/{EMBEDDING_MODEL_ID}", token=token))
    if _without_top_keys(model_after, frozenset({"updated_at"})) != \
            _without_top_keys(expected_model_after, frozenset({"updated_at"})):
        raise RuntimeError("embedding model full readback mismatch")
    checkpoint("model-api-postwrite-readback", {
        "after_sha256": sha256(canonical_bytes(model_after)), "only_base_url_changed": True,
        "custom_header_names": sorted((model_after["parameters"].get("custom_headers") or {}).keys())})
    return {
...
def apply(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        pg = r.inspect("container", PG_CONTAINER)
        assert source_app and source_doc and pg
        if not source_app["State"]["Running"] or not source_doc["State"]["Running"] \
                or not pg["State"]["Running"]:
            raise RuntimeError("a source runtime dependency stopped since preflight")
        if source_app["Image"] != APP_IMAGE or source_doc["Image"] != DOCREADER_IMAGE:
            raise RuntimeError("source image drift since preflight")
        source_env = env_map(source_app)
        validate_apply_source_environment(source_env, preflight_value.get("source", {}))
        binding = source_pg_binding(source_app, pg, source_env)
        if binding != preflight_value["source"]["postgres_binding"] \
                or pg.get("Image") != preflight_value["source"]["postgres_image"]:
            raise RuntimeError("source PostgreSQL container/image/network binding drift since preflight")
        if database_identity(r, SOURCE_DB) != {"database": SOURCE_DB, "user": DB_USER}:
            raise RuntimeError("source database connection identity drift since preflight")
        parser_env = preflight_value.get("source", {}).get("parser_env")
        validate_apply_parser_environment(parser_env)
        if build_parser_environment(env_map(source_doc)) != parser_env:
            raise RuntimeError("source DocReader parser environment drift since preflight")
        ...
        restored_identity = database_identity(r, TARGET_DB)
        if restored_identity != {"database": TARGET_DB, "user": DB_USER}:
            raise RuntimeError("restored target database/user identity mismatch")
        restored = source_snapshot(r, TARGET_DB)
        if restored != before_source:
            raise RuntimeError("restored DB source/job identity mismatch")
        state.checkpoint("restored-target-database", {
            "snapshot_sha256": sha256(canonical_bytes(restored)), "identity": restored_identity})

        if database_identity(r, TARGET_DB) != {"database": TARGET_DB, "user": DB_USER}:
            raise RuntimeError("target database/user identity mismatch before narrow SQL")
...
"""
        ...
        r.docker("start", SEED)
        expected_copy = {rel: {"size": size, "sha256": digest}
                         for rel, size, digest in EXPECTED_FILES}
        copy_tar(r, SOURCE_APP, "/data/files", expected_copy, SEED, "/data/files")
        for rel, size, digest in EXPECTED_FILES:
        ...
        validate_env_mapping(private_env)
        env_bytes = "".join(f"{k}={v}\n" for k, v in private_env.items()).encode()
        redis_env_bytes = f"REDIS_PASSWORD={redis_password}\n".encode()
        secrets_staged = True
        remote_secret_file(r, REMOTE_APP_ENV, env_bytes)
        remote_secret_file(r, REMOTE_REDIS_ENV, redis_env_bytes)
        ...
        r.docker("create", "--pull=never", "--name", REDIS, "--network", NETWORK,
                 ...
                 "--env-file", REMOTE_REDIS_ENV, "--entrypoint", "/bin/sh", REDIS_IMAGE,
        ...
        r.docker("create", "--pull=never", "--name", APP, "--network", NETWORK,
                 ...
                 "--env-file", REMOTE_APP_ENV,
                 "--publish", f"127.0.0.1:{APP_PORT}:8080",
                 "--mount", f"type=volume,src={FILES_VOLUME},dst=/data/files",
                 "--mount", f"type=volume,src={DOCREADER_TMP_VOLUME},dst=/tmp/docreader,readonly",
                 APP_IMAGE)
        cleanup = cleanup_remote_secret_files(r)
        if not cleanup["clean"]:
            raise RuntimeError("private runtime environment cleanup failed")
        secrets_staged = False
        state.checkpoint("removed-private-runtime-environment-files", cleanup)
        ...
        api_evidence = configure_api(args.private_auth, state.checkpoint)
        ...
        stop_evidence = stop_and_inspect_targets(r)
        cleanup_evidence = cleanup_remote_secret_files(r) if secrets_staged else {
            "attempted": False, "clean": True, "reason": "no-staged-secret-files"}
        # Do not include command stderr or secrets.
        state.stop(f"{type(exc).__name__}: {exc}", stop_evidence, cleanup_evidence)
        raise
