...
GUARD_UPSTREAM = "https://dashscope.aliyuncs.com/compatible-mode/v1/embeddings"
EXTERNAL_WINDOW_SECONDS = 1440.0
...
EXISTING_CHUNK_CAPTURE_SHA256 = "3b14ef391cad1a215a8012acbfb385b0093425aef60c116eebbd0a0a81b54924"
...
OLD_MATERIAL_IDS = tuple(OLD_KNOWLEDGE_IDS)
...
def load_existing_revision_captures(evidence: Path) -> dict[str, dict[str, Any]]:
    capture_raw = regular_file(
        evidence / "existing-chunk-capture.json", EXISTING_CHUNK_CAPTURE_SHA256
    )
    capture = json.loads(capture_raw)
    rows = capture.get("documents")
    if (capture.get("contract") != "830-g3-existing-chunk-capture.v1"
            or capture.get("status") != "PASS"
            or capture.get("provider_calls") != 0
            or capture.get("writes_to_application") != 0
            or not isinstance(rows, list) or len(rows) != 4):
        raise RuntimeError("existing chunk capture manifest drift")
    by_id = {row.get("material_id"): row for row in rows}
    if set(by_id) != set(OLD_MATERIAL_IDS):
        raise RuntimeError("existing chunk capture membership drift")
    result: dict[str, dict[str, Any]] = {}
    evidence_root = evidence.resolve()
    for mid in OLD_MATERIAL_IDS:
        row = by_id[mid]
        relative = Path(row.get("path", ""))
        if relative.is_absolute() or ".." in relative.parts:
            raise RuntimeError("existing revision capture path escapes root")
        path = (WORKTREE / relative).resolve()
        if not path.is_relative_to(evidence_root):
            raise RuntimeError("existing revision capture path escapes evidence")
        value = json.loads(regular_file(path, row.get("capture_sha256")))
        descriptor = value.get("descriptor")
        if not isinstance(descriptor, dict):
            raise RuntimeError("existing descriptor missing")
        manifest = descriptor.get("chunk_manifest") or {}
        expected = {
            "knowledge_id": row.get("knowledge_id"),
            "parse_attempt": row.get("parse_attempt"),
            "file_sha256": row.get("file_sha256"),
            "chunk_count": row.get("chunk_count"),
            "manifest_digest": row.get("manifest_digest"),
        }
        actual = {
            "knowledge_id": descriptor.get("knowledge_id"),
            "parse_attempt": descriptor.get("parse_attempt"),
            "file_sha256": (descriptor.get("file_digest") or {}).get("value"),
            "chunk_count": manifest.get("chunk_count"),
            "manifest_digest": manifest.get("digest"),
        }
        if (actual != expected or row.get("post_descriptor_unchanged") is not True
                or row.get("knowledge_id") != OLD_KNOWLEDGE_IDS[mid]):
            raise RuntimeError("existing revision capture identity drift")
        result[mid] = descriptor
    return result
...
def load_frozen_inputs(root: Path) -> FrozenInputs:
    ...
    old = load_existing_revision_captures(evidence)
    old_materials = {}
    for mid in OLD_MATERIAL_IDS:
...
def validate_runtime_precondition(provision: dict[str, Any], app_inspect: dict[str, Any],
    ...
    doc_env = parse_env(doc_inspect.get("Config", {}).get("Env", []))
    ...
    if (app_env.get("BATCH_EMBED_SIZE") != "100" or app_env.get("REDIS_DB") != "0"
            ...
            or app_env.get("REDIS_USERNAME") != ""
            or app_env.get("STORAGE_TYPE") != "local"
            or app_env.get("LOCAL_STORAGE_BASE_DIR") != "/data/files"
            or app_env.get("DOCREADER_ADDR") != "docreader-g3:50051"
            or app_env.get("DOCREADER_TRANSPORT") != "grpc"):
        raise RuntimeError("app batch/redis config drift")
    redis_password = redis_env.get("REDIS_PASSWORD")
    if redis_password in (None, "") or app_env.get("REDIS_PASSWORD") != redis_password:
        raise RuntimeError("app/redis password binding drift")
    expected_doc_env = {
        "DOCREADER_GRPC_PORT": "50051", "DOCREADER_ODL_HYBRID": "off",
        "DOCREADER_EXTERNAL_HTTP_PROXY": "", "DOCREADER_EXTERNAL_HTTPS_PROXY": "",
        "DOCREADER_ODL_HYBRID_URL": "",
    }
    actual_doc_env = {key: value for key, value in doc_env.items() if key.startswith("DOCREADER_")}
    if actual_doc_env != expected_doc_env:
        raise RuntimeError("docreader env binding drift")
    app_ports = app_inspect.get("HostConfig", {}).get("PortBindings") or {}
    if app_ports != {"8080/tcp": [{"HostIp": "127.0.0.1", "HostPort": "18294"}]}:
        raise RuntimeError("app published port binding drift")
    for name, inspect in ((DOCREADER, doc_inspect), (REDIS, redis_inspect)):
        if inspect.get("HostConfig", {}).get("PortBindings") not in (None, {}):
            raise RuntimeError(f"unexpected published port: {name}")

    def mounts(inspect: dict[str, Any]) -> set[tuple[str, str, str, bool]]:
        rows = inspect.get("Mounts")
        if not isinstance(rows, list):
            raise RuntimeError("runtime mount inventory missing")
        result = {(row.get("Type"), row.get("Name"), row.get("Destination"), bool(row.get("RW")))
                  for row in rows}
        if len(result) != len(rows):
            raise RuntimeError("duplicate runtime mount")
        return result

    expected_app_mounts = {
        ("volume", FILES_VOLUME, "/data/files", True),
        ("volume", DOCREADER_TMP_VOLUME, "/tmp/docreader", False),
    }
    expected_doc_mounts = {
        ("volume", DOCREADER_TMP_VOLUME, "/tmp/docreader", True),
    }
    if mounts(app_inspect) != expected_app_mounts or mounts(doc_inspect) != expected_doc_mounts:
        raise RuntimeError("runtime mount binding drift")
    if mounts(redis_inspect):
        raise RuntimeError("redis mount binding drift")
    if any("weknora-g2" in str(row.get("Name", ""))
           for inspect in (app_inspect, doc_inspect, redis_inspect)
           for row in (inspect.get("Mounts") or [])):
        raise RuntimeError("G2 runtime mount forbidden")
...
def validate_guard_ledger(ledger: dict[str, Any], completed: list[Material]) -> None:
    ...
    header = {key: ledger.get(key) for key in (
        "contract", "upstream", "manifest_sha256", "max_attempts", "stopped"
    )}
    if header != {
        "contract": "830-g3-embedding-attempts.v1",
        "upstream": GUARD_UPSTREAM,
        "manifest_sha256": MANIFEST_SHA256,
        "max_attempts": 11,
        "stopped": False,
    } or not isinstance(attempts, list):
        raise RuntimeError("provider attempt failed: guard ledger header drift")
...
def validate_old_source_id(material_id: str, receipt: dict[str, Any]) -> None:
    expected = OLD_SOURCE_IDS.get(material_id)
    if expected is not None and receipt.get("revision_source_id") != expected:
        raise RuntimeError(f"old source identity drift: {material_id}")
...
class ExternalDeadline:
    def __init__(self, *, clock: Callable[[], float] = time.monotonic,
                 seconds: float = EXTERNAL_WINDOW_SECONDS,
                 started_at: float | None = None):
        if seconds != EXTERNAL_WINDOW_SECONDS:
            raise RuntimeError("external window must reserve exact guard shutdown margin")
        self.clock = clock
    ...
    def remaining(self) -> float:
        ...
        if value <= 0:
            raise TimeoutError("external window deadline exceeded")
        return value
    ...
    def sleep(self, duration: float, sleeper: Callable[[float], None]) -> None:
...
class UploadExecutor:
    def __init__(self, api: LoopbackAPI, state: RunState, artifacts: Path,
                 frozen: FrozenInputs, guard_ledger_reader: Callable[[float], dict[str, Any]],
                 sleep: Callable[[float], None] = time.sleep,
                 deadline: ExternalDeadline | None = None):
        ...
        self.seen_knowledge_ids = set(OLD_KNOWLEDGE_IDS.values())

    def _timeout(self, requested: float) -> float:
        return requested if self.deadline is None else self.deadline.timeout(requested)

    def _sleep(self, duration: float) -> None:
        if self.deadline is None:
            self.sleep(duration)
        else:
            self.deadline.sleep(duration, self.sleep)

    def _guard_ledger(self) -> dict[str, Any]:
        timeout = self._timeout(120)
        value = self.guard_ledger_reader(timeout)
    ...
    def upload(self, material: Material) -> dict[str, Any]:
        ...
        knowledge_id = row["id"]
        if knowledge_id in self.seen_knowledge_ids:
            raise RuntimeError("upload returned old or repeated knowledge id")
        self.seen_knowledge_ids.add(knowledge_id)
    ...
    def wait_completed(self, knowledge_id: str, material: Material,
                       max_polls: int = 180, interval: float = 2.0) -> dict[str, Any]:
        for poll in range(max_polls):
            value = self.request_json("GET", f"/api/v1/knowledge/{knowledge_id}",
                                      f"wait-{material.material_id}-{poll}")
            row = value.get("data") or {}
            if (row.get("id") != knowledge_id or row.get("tenant_id") != TENANT_ID
                    or row.get("knowledge_base_id") != RAW_KB_ID
                    or row.get("file_name") != material.filename
                    or row.get("file_size") != material.size
                    or row.get("file_hash") != material.md5
                    or row.get("file_sha256") != material.sha256):
                raise RuntimeError("knowledge readback binding drift")
            status = row.get("parse_status")
            if status == "completed":
                if not isinstance(row.get("current_parse_attempt"), int) \
                        or isinstance(row.get("current_parse_attempt"), bool) \
                        or row["current_parse_attempt"] <= 0:
                    raise RuntimeError("knowledge attempt binding drift")
                return row
    ...
    def chunks(self, knowledge_id: str, attempt: int, label: str) -> list[dict[str, Any]]:
        ...
        while page <= 100:
            ...
            if any(not isinstance(row, dict)
                   ...
                   or row.get("knowledge_base_id") != RAW_KB_ID
                   or row.get("knowledge_id") != knowledge_id
                   or row.get("parse_attempt") != attempt for row in data):
                raise RuntimeError("revision chunk scope drift")
            rows.extend(data)
            total = value.get("total")
            if not isinstance(total, int) or len(rows) > total:
    ...
    def capture_new(self, material: Material, knowledge_id: str, expected_attempt: int) -> dict[str, Any]:
        before = self.revision(knowledge_id, f"revision-before-{material.material_id}")
        if before.get("knowledge_id") != knowledge_id or before.get("parse_attempt") != expected_attempt:
            raise RuntimeError("revision identity drift")
        chunks = self.chunks(knowledge_id, before["parse_attempt"], f"chunks-{material.material_id}")
        if any(row.get("tenant_id") != TENANT_ID or row.get("knowledge_base_id") != RAW_KB_ID
               or row.get("knowledge_id") != knowledge_id
               or row.get("parse_attempt") != expected_attempt for row in chunks):
            raise RuntimeError("revision chunk scope drift")
    ...
    def run_new(self, material: Material) -> dict[str, Any]:
        ...
        completed_row = self.wait_completed(row["id"], material)
        ledger = self._guard_ledger()
        validate_guard_ledger(ledger, self.completed + [material])
        descriptor = self.capture_new(material, row["id"], completed_row["current_parse_attempt"])
...
def validate_authorization(path: Path, runner_sha: str, provision_sha: str) -> dict[str, Any]:
    ...
    all_ids = list(MATERIAL_IDS) + list(OLD_MATERIAL_IDS)
    expected = {"contract": "830-g3-source-upload-authorization.v1", "decision": "APPROVED",
                ...
                "action_scopes": {
                    "upload": {"material_ids": list(MATERIAL_IDS), "max_actions": 11},
    ...
    if set(value) != set(expected) | {"approved_at"}:
        raise RuntimeError("external-send authorization shape drift")
    if any(value.get(key) != expected_value for key, expected_value in expected.items()):
        raise RuntimeError("external-send authorization mismatch")
    approved_at = value.get("approved_at")
    if not isinstance(approved_at, str):
        raise RuntimeError("external-send authorization approved_at invalid")
    try:
        parsed = dt.datetime.fromisoformat(approved_at)
    except ValueError as error:
        raise RuntimeError("external-send authorization approved_at invalid") from error
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise RuntimeError("external-send authorization approved_at lacks timezone")
    return value
...
class DockerRuntime:
    ...
    def __init__(self, command: list[str]):
        expected = ["colima", "ssh", "--profile", "default", "--", "sudo", "docker"]
        if command != expected:
            raise RuntimeError("docker command must be exact default Colima transport")
        self.command = command
        self.guard_started = False
        self.egress_created = False
        self.egress_connected = False

    def call(self, args: list[str], *, input_bytes: bytes | None = None, check: bool = True,
...
""" % MANIFEST_SHA256
    ...
    def read_guard_ledger(self, timeout: float = 120) -> dict[str, Any]:
        result = self.call(["exec", APP, "cat", "/run/g3-830-upload/ledger.json"], timeout=timeout)
        value = json.loads(result.stdout)
        if not isinstance(value, dict):
            raise RuntimeError("guard ledger is not object")
        return value

    def copy_guard_ledger(self, local: Path, timeout: float = 120) -> None:
        result = self.call(["exec", APP, "cat", "/run/g3-830-upload/ledger.json"], timeout=timeout)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        dummy = UploadExecutor(api, state, args.artifact_dir, frozen,
                               lambda *_: load_json(args.artifact_dir / "guard-ledger.json"))
        ...
        guard_launch_started = time.monotonic()
        runtime.start_guard(staged)
        external_deadline = ExternalDeadline(started_at=guard_launch_started)
        validate_guard_ledger(
            runtime.read_guard_ledger(external_deadline.timeout(120)), []
        )
        ...
        executor = UploadExecutor(api, state, args.artifact_dir, frozen,
                                  runtime.read_guard_ledger, deadline=external_deadline)
        execute_material_sequence(frozen.materials, state, executor.run_new)
        ledger = executor._guard_ledger()
        validate_guard_ledger(ledger, list(frozen.materials))
        runtime.copy_guard_ledger(
            args.artifact_dir / "guard-ledger-final.json", executor._timeout(120)
        )
        ...
        for mid in OLD_MATERIAL_IDS:
            ...
            validate_old_source_id(mid, receipt)
