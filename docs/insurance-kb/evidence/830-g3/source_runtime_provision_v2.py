...
def source_pg_binding(source_app: dict, pg: dict, app_env: dict[str, str]) -> dict:
    ...
    if not host or port != "5432":
        raise RuntimeError("source DB_HOST/DB_PORT identity drift")
    ...
    return {"host": host, "port": port, "network": matches[0],
...
def preflight(args: argparse.Namespace) -> None:
    ...
    unknown_doc_env = {k for k in doc_env if k.startswith("DOCREADER_")
                       and k not in DOCREADER_ENV_ALLOWLIST and k not in DOCREADER_ENV_FORCED_EMPTY}
    if unknown_doc_env:
        raise RuntimeError("source DocReader has an unreviewed environment key")
    parser_env = {k: v for k, v in doc_env.items() if k in DOCREADER_ENV_ALLOWLIST}
    ...
    validate_env_mapping(parser_env)
