## Task3bj — isolate unavailable PDF character locations (2026-09-16)

User explicitly requests repairing fresh acceptance and preventing local parsing errors from aborting the document. Root sole writer; independent locator reviewer checks frozen changes. Existing G3-AUTO-2/3/4/6, same services/DB/models. Actual RED: fresh run384f1c4c source fails four times on2105-1 rate-table page2 char0. PDFium returns readable Unicode but y700..705 outside page height595.25 (media/crop both zero-origin). No coordinate transform or guessed rectangle is justified.

Reuse native PDFium capture, durable native artifacts, shared Go evidence index and existing Harness field validation. Complete captures MUST retain byte-identical v1 schema/producer/identity. Only partial captures emit builtin-pdfium-native-locators.v2 / producer.v2 with per-page unavailable_ranges containing exact non-whitespace codepoint ranges and finite reason codes. Text/pages/hash/valid boxes stay unchanged. Both Go validators require boxes + explicit gaps to cover every visible character exactly once; missing/overlapping/unexplained coverage still rejects. Harness validates the same gap contract. Existing quote resolvers reject a quote crossing gaps, while unaffected quotations remain eligible; existing field validation isolates ordinary failures. No invented page-wide box, text deletion, candidate patch, page-only authority, or second parser/model pipeline.

Contain character bbox errors and ambiguous native character mapping at the affected character/page; rotated-page native text may remain with unavailable locations. Keep unreadable bytes, invalid Unicode, ambiguous source/page integrity and untrusted scanned-text failures explicit rather than silently claiming complete content. Deterministic parser-returned errors should keep the existing terminal result path instead of being wrapped as a retryable transport error. Genuine transport failures remain bounded-retry. Record native availability in existing metadata/artifact; no new store.

Write scope: docreader/parser/pdf_parser.py + tests; internal/types native locator coverage helper and tests; internal/infrastructure/docparser/grpc_parser.go + tests; internal/application/service/concept_source_authority_830_g2.go + tests; Harness product_ingestion/source_geometry.py + tests; original spec/plan/evidence. Delivery helpers only if required to bound existing component builds. No unrelated UI/performance expansion. Source-failure UI accounting is separately retained as an acceptance defect, not hidden by parser success.

- [ ] RED producer bad bbox/unavailable/mapping/rotation preserve source and other pages; Go transport/index accept explicit gaps but old v1 malformed remains rejected; Harness rejects malformed declarations even on unselected pages.
- [ ] Implement shared coverage contract, backward-compatible producer and consumer changes; preserve deterministic parser error result for terminal handling.
- [ ] GREEN focused suites plus actual108-page offline PDF diagnostic, no import/embedding/publication from diagnostics. Freeze and independent review.
- [ ] Deploy only affected existing components with memory-bounded builds, health including DocReader. Recover through webpage using existing successes; document this as recovery, never fresh acceptance. Then freeze runtime before fresh full-flow acceptance.

## Task3bg — offered legal issuer and explicit recorded-identity retry

This is the implementation plan for the already authorized G3 goal; no business stage may be executed by Codex. Root remains sole writer; g3_admission_finish reviews. Readonly boundary review /private/tmp/g3-task3bf-parallel-development-boundary-review.md (b88d92d37068b9151827a9bfafe326af7e3c1491e063183f266cfd02599a63f1) permits Python/docs development while frozen APP111 builds. Keep every APP input identical to immutable source111; approved non-APP commits may advance HEAD while its exported build continues. The APP helper compares cumulative diff from111, admits only exact non-APP paths, proves111 remains an ancestor, and records the integration HEAD and undeployed differences. Independent reviewer approved this delivery boundary on2026-09-16; no business or build-recipe change. No new environment/DB/service or model choice.

Confirmed RED evidence: fresh webpage run4bb03c2e-4b5a-4e03-94f8-acbac57bf31a source/routing succeed; identity returnsannuity, company full/short variants and no2.0 version; needs_confirmation with original response retained. The terms' saved source snapshot chunks4/5 contain the full legal issuer; broad selector incorrectly treats first-page“保险人就是保险公司” as sufficient. Public CheckpointStore refuses every dispatched model call beyond the successful prefix; legacy recovery tests monkeypatch an unused public path, so they cannot prove webpage recovery.

Reuse shared identity source selector. Prefer legal-entity company suffix candidates among currently eligible page-one/<=3-page offered ranges; a generic first-page phrase must not suppress an available later legal company block. Preserve original broad fallback where no legal candidate exists, one auxiliary block bound, same-material locators, name/type/role page-one constraint and source-range geometry. This only offers source text to the model; no alias rewrite, canonical issuer rule, raw response modification or admission waiver. Existing Task3be prompt remains.

Reuse the one public CheckpointStore/API/button and existing worker. A new CheckpointPlan/Receipt v3 records an explicit retry-call reference separately from reused calls; workflow stays2. Only an intact fully recorded terminal semantic identity failure with successful uploads/source/routing prefix and no later work qualifies. Reference original failed stage/job/generation/dependency plus recorded_identity_reference's current scope/request/raw/hash/input/prompt/base proof. Unknown, diagnostic, interrupted, missing/corrupt bytes, multiple calls, foreign scope, active jobs and unrelated later-stage failures remain ineligible. No automatic retry is introduced: clicking existing恢复处理 must create one new idempotent child, expected_version fenced.

Checkpoint verification revalidates the preserved source/routing artifacts, current original files/parse revisions and complete failed-call bytes/failure producer state under existing locked final job fence. It does not count the failed identity as a successful/reused stage or reused model call. Existing identity handler naturally calls normal execute_stage_call after successful checkpoint with fresh run/job/call identity and current offered context. Do not reactivate legacy recovery routes or clone raw responses. Failed-checkpoint explicit retry inherits the retry reference until new identity runs; successful/newly failed identity uses its actual next outcome, rather than an unbounded flat ancestry list.

Compatibility: v1/v2 exact JSON bytes/hashes/child IDs remain identical; absent/new-empty fields must be omitted for old contracts and nonempty retry refs rejected there. v3 contract revision and artifact contract_version must be3 even though workflow_version remains2; receipt metadata is handled explicitly for new v3 only. Keep128KiB references-only bound and duplicate/intersection rejection. Original old runtime can roll back before any v3 work; after v3 creation, do not claim downgrade compatibility.

Write scope: product_ingestion/{identity,checkpoints,checkpoint_store,checkpoint_artifacts}.py, only necessary existing pipeline receipt metadata assignment, focused existing identity/checkpoint/API tests, original OpenSpec/plan/evidence. No Go/model executor/store ledger/migration/new endpoint. Existing public button remains the explicit operation; describe its identity re-evaluation through existing capability presentation if a minimal label change is needed, without introducing another approval flow.

Tests before implementation: generic first-page plus genuine later company offered consistently; no falsecross-page-tail inclusion, no invented full issuer. Real public admission + HTTP fixture without legacy monkeypatch: intact blocked semantic identity exposes recovery and duplicate clicks yield one child. Worker forbids parse/capture/embedding/source rerouting, preserves original PDFs/snapshots/raw/terminal, rechecks current source, sends exactly one new identity call, counts failed original only as provenance. Negative raw/request digest/job generation/stage/dependency/source/parse/scope/terminal/unknown tests; old v1/v2 exact encoded bytes; v3 metadata revision; inherited failed-checkpoint retry. Split misleading existing“incomplete recorded”fixture into actual incomplete negative and new fully recorded positive. Retain current normal incremental recovery/validation tests.

After GREEN and frozen independent review, upgrade only same Harness (UI only if genuinely changed), restore through webpage once, observe platform's own terminal/publication/search/evidence and usage. No Codex business scripts/field completion/candidates/manual publication. This recovery never becomes an uninterrupted fresh acceptance. Then another genuinely unprocessed product/three originals must complete from webpage with no code/deployment/manual continuation during that acceptance, measured from last upload to searchable release and evidence.

## Task3bf — reuse proven first-parse bindings on resident citation reads (2026-09-16)

User reports slow local field/PDF reads. Task3az directory/PDF reuse and Task3bb terminal-history scan are deployed, but live preview still took2.835631 seconds. Readonly call-graph diagnosis /private/tmp/g3-first-parse-warm-read-diagnosis.md confirms each resident source read repeats typed first-record decoding, full serialization and canonical chunk-binding reconstruction. Live artifact sizes reach122MB; this is demonstrated repeated work, not measured attribution of the entire preview latency.

Root sole writer, existing g3_admission_finish readonly reviewer; existing G3-AUTO-3/6. Scope only internal/application/service/{concept_source_reuse_830_g3.go,g3_first_parse.go,g3_first_parse_test.go}, original plan/spec and evidence. Reuse the existing16-entry source cache. Cold reads retain complete signature, payload, typed-contract and chunk-binding validation. Only after both first/source validation succeed may a private resident proof bind the owned prepared record pointer and first payload digest. Warm loads must still reopen first artifact, verify current signing key/domain/key/signature and exact payload SHA; only then may the same proved owned record skip typed decode/re-serialization/chunk reconstruction. A copied/unproved entry takes full validation. Keep legacy absence checks, first-file appearance, deletion/tampering and key revocation checks per request. No stat-only trust, file format change, persistent cache, model/DocReader call, new service or DB change.

RED: real resident load vs full first validation on the same signed multi-chunk fixture demonstrates equivalent locator/data but no allocation reduction on old code. GREEN must reduce resident allocations relative to full validation; add copied/unproved changed-binding rejection and same-size warm tamper with restored mtime. Retain existing first-parse/source reuse authority tests, key removal, restart, legacy appearance and resource revocation. After independent frozen review, build/deploy only same APP when active tasks permit, then measure actual webpage field and first/repeated PDF previews. Keep latency status open if actual improvement is insufficient; fixture speed is not live acceptance. Task3be identity recovery remains an independent platform gap and is not hidden by this repair.

## Task3be — explicit issuer and edition instructions (2026-09-16)

The fresh webpage run4bb03c2e-4b5a-4e03-94f8-acbac57bf31a parsed three originals and classified annuity successfully, then stopped identity resolution: model selected both full issuer and shorthand, and omitted explicit2.0 edition from all version_label fields. Original response/inputs remain in ProductStageModelCall. This actual semantic failure is RED; no fabricated candidate or corrected response may replace it.

Root sole writer, g3_admission_finish readonly reviewer; existing G3-AUTO-1/3/6. Narrow scope: product_ingestion/identity.py instructions plus existing spec/plan/evidence only. Keep source selection, exact evidence locators, response schema, deterministic conflict checks and mandatory version/filing contracts unchanged. Tell the existing identity model to prefer the full legal issuer where offered within the same material; do not invent issuer from another material. Explicit edition tokens in the formal name (numeric editions such as2.0, V2 or clearly labelled year editions) must be copied as version_label with local version-purpose evidence. Genuine absent/conflicting anchors remain unresolved. No hardcoded company aliases, product-specific handling or loosening admission.

Run existing identity/adapter/routing tests, freeze and independently review. Only same Harness deployment after this completed identity run; retain original rejected upload run awaiting its existing deadline, no manual status mutation. Recover via platform UI if available, reusing parsed sources; otherwise report missing recovery capability rather than using a script. This is a failed first-upload acceptance followed by repair/recovery, never relabel it an uninterrupted fresh-flow PASS. A subsequent fresh acceptance is still required if the recovery works.
