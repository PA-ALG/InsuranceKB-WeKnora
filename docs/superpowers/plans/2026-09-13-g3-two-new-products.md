...
用户明确：不要求所有字段补全，合理缺失可接受，本阶段不在缺失或字段质量上继续投入过多时间，核心跑完流程。因此不再追加补字段/质量优化轮次；已有 unknown/待补充 为可接受终态，不把缺失本身作为发布阻断。当前已准备D8允许unknown，随后仅完成必要结果检查、实际候选/本机审核发布和页面/原文回查。保留原字段、失败原因和原始响应，不把无证据的断言当成已确认。千份压测/质量提升不从本轮FLOW结果推导完成。
