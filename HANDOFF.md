...
> 最后更新：2026-07-21（021/024/Wave 2 fast-follow 已分别随 PR #23/#13/#22 合入 `main`；020 T1 的 PR #24 合并前硬化已关闭四项 P1，canonical admission 仍按设计 BLOCKED。031 的 T1～T8 本地软件收口已完成，但外部签名/cap、root-owned trust store 与人工 clean identity 尚未满足；见 0j/0k/0l/B22/B23/B24。）
...
0k. **OpenSpec 020 T1 run-admission 软件闭环（2026-07-21，PR #24 合并前硬化）**：已按 SDD/TDD 实现类型化 admission/evaluator/CLI、签名 canonical identity、完整输入与 execution-surface 指纹、安全 provider probe、durable budget reservation/request-attempt ledger、每产品 runtime 重验、annotation/baseline 可恢复执行入口、提交前 exact-bytes/语义回验及 validation→settlement 同锁。最终高风险复核关闭四项 P1：删除可伪造的 provider `no_usage` 退款路径（历史态只按 uncertain/full-reserve 恢复）；同 run revision 只允许提高 account ceiling，禁止新增/删除/修改产品、请求或池；production Bailian 必须使用与真实请求 `model` 相同的 provider-guaranteed immutable deployment ID；019/021 固定实际 main merge `4d9c84e`/`cfefcc9b`，拒绝 feature head/cherry-equivalent 替代。初始 hardening 为 `2169c582`；最新 main 合流后将 024 新增/修改的 16 个 execution files 全部重新指纹化，最终 clean source-plan commit 为 `59695273`，canonical JSON/Markdown 再从该 SHA 生成，结果不含 dirty/dependency/identity drift blocker。合流后本地 focused **700 passed**、deterministic **2706 passed/30 deselected**、Ruff/mypy 297 files/OpenSpec 全绿。权威工件仍为零模型 **`BLOCKED`**，runtime capability=`budget-ledger-v3-canary-v1`，provider probe 全为 `not_attempted`，模型调用/token/费用均为 0；剩余阻塞只有预算、11 个产品历史 provenance、一个产品缺失/未消费输入、签名审批及三模型不可变 identity。**这只表示 T1 软件闭环完成，不表示 020 数据运行完成**：T2～T7/D4b 尚未运行，D5/T8 最终对账也未完成；得到 READY 后方可执行真实模型任务。

0l. **OpenSpec 031 operational run admission（2026-07-21，T8 本地软件收口完成）**：T1～T8 本地软件已完成，覆盖输入规范化/clean identity 算法、legacy provenance Git evidence、离线 Ed25519 authority、BudgetLedger v5 的 pre-POST infrastructure reserve、crash-safe 百炼控制器、签名 pricing/shared provider cap/cleanup 与 new/adoption 编排；整个实现保持 fail-closed，未执行 provider mutation 或推理。T2.3 仍因仓库没有唯一历史 session-agent ID 而 **BLOCKED**，不得把批次标签伪造成 agent 身份；production identity 也必须在人工 clean commit 后重算。真实 provenance/budget/provisioning/adoption/pricing/shared provider-cap/cleanup 签名、root-owned trust store 与 provider cap 均未提供，因此不能声称 canonical admission `READY`。两套资源 `qwen3.7-plus-2026-05-26-031strng`、`deepseek-v4-flash-031weak1` 只保留 **2026-07-21 last observed RUNNING** 的历史事实，约 **¥11.04/小时** 持续费用风险未解除；本轮不刷新远端状态，也不创建/采纳/删除。最终 fresh 证据：031 focused **248 passed**、020 admission **711 passed**、deterministic **2965 passed / 30 deselected**、Ruff、mypy **309 files**、031/020 OpenSpec strict 与 diff/secret 审计全绿；详见 `openspec/changes/031-operational-run-admission/validation-report.md`。合并顺序：031 独立收口/人工提交/重算 identity 后先合并，再 rebase/recompute 002，禁止同提交混入。执行教训：高风险闭环必须分段 RED；agent/tool 60 秒无输出即轮询并回报；focused 绿后仍须跑全量，因为全量才会揭示旧迁移 fixture 与新 schema 的组合问题。
