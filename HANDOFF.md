...
> 最后更新：2026-07-21（021 已随 PR #23、024 已随 PR #13 合入 `main`；关键路径进入 020，T1 零模型 run-admission 由 PR #24 承载且真实标注/baseline 尚未运行。Wave 2 三版规格已随 PR #12 合入，PR #22 正在收口 H1.3a 独立漂移维度与认领状态；各 change 仍按 L4 硬依赖分别认领，不作整波无条件放行。）
...
0g. **OpenSpec 023 本机 WeKnora live 环境（2026-07-17 当前状态）**：PR #10、#16、#19、#20 均已合入 `main`。受信 main workflow 已从锁定的 Tencent upstream `5eefa70e...` 构建 `linux/arm64` app，OCI/GitHub attestation、SLSA provenance 与 SPDX SBOM 均已核验；`deploy/local-live/images.lock` 和 Compose 固定 subject digest `sha256:e2dd00b37dbcfebf87fab9d1e2338ad43e6ea9939a5ba9fcab9d412d866521f5`。本机六服务 healthy，宿主只监听 `127.0.0.1:8080/8081/5442`；百炼五角色 direct probe、幂等 provision、普通 PDF 与 clean-SHA VLM smoke 均通过，VLM 结果为 `status=completed / image_ocr_chunks=1 / image_caption_chunks=1 / dirty=false / evidence=exact`。PR #20 另以真实 API TDD 修复重复注册、Wiki indexing strategy、pages envelope 与服务端扩展 VLM overrides 四项合同差异；最终 live 又发现 Wiki 空关系返回 `null`，PR #9 以 `test_s2_4_*` RED→GREEN 仅规范化为 `[]`。018 的本机 5-node 已 `5 passed / skipped=0`；正式 GitHub exact-SHA gate 作为收尾 PR 的合并前门禁执行。
   ...
   **收敛轨 — 021 Source lifecycle ordering（T1～T10 全部完成；硬依赖 018✅）**
   ...
   - [ ] 人工 owner 复核后 commit/push/PR（AI 不越过 Git 边界）。
...
0h. **并行执行蓝图（总设计师制定，业务方 2026-07-16 拍板）**：全轨道结构（L0 解阻塞 / L1 关键路径 021→020 / L2 工作台 008 / L3 MCP 013 / L4 知识形态 010→009→012 / L5 抽取提准 024 / L6 治理）、优先级与护栏见 `docs/insurance-kb/22-parallel-execution-blueprint.md`；**change 号与 Alembic 迁移号一律先在 `openspec/changes/README.md` 注册表占号**（022 撞号教训；0006=021、0007=010、0008=009、0009=012 已预分配）。018/019/023 已完成，当前关键路径为 021→020；013 与 008 W4 已随 PR #9 解锁。010 的 knowledge 域段（T5 起）仍排在 021 之后。**Wave 2（009/011/012）已条款化三版（正式 delta 过 strict；迁移 0008/0009/0010 已占号；冻结投影扩展/断言级绑定/三方对账等合同见各 spec），规格复审完成（PR #12 已合入，2026-07-18），按 L4 依赖顺序可认领**（非阻断 fast-follow：H1.3a 维度可并存已修订）。
...
0j. **OpenSpec 021 主流程收敛（2026-07-19，软件完成）**：确定性链路已贯通 `SourceRevision → per-source advisory lock → SourceHead/append-only SourceEvent → notify/import/delete → stale-or-blocked audit no-op → strictly-newer reactivate`；历史 ordering 不可证明时只建唯一 open BackfillIssue，显式 resolver 与正常 lifecycle 共锁且 exact retry 返回原 resolution event。Task8 新增 13 个真实业务并发节点与 8 个真实 Alembic 节点；本机 Compose 私网 PG16 核心 **21 passed/skipped=0**，最新全 `integration_postgres` **25 passed/skipped=0**，CI lane 合同 **39 passed**。最终 OpenSpec strict、Ruff、mypy **248 files**、deterministic **1901 passed/30 deselected**、single Alembic head `0006` 与 fresh SQLite roundtrip/check 全绿；独立 spec=PASS、quality=APPROVED。宿主 sandbox 拒绝 `127.0.0.1:5442` 后没有循环重试，而是复用已有 `python:3.12-slim` 作为临时 Compose runner，通过 `local-live-harness-db` 私网执行；该做法保留现有数据库且不打印 DSN。当前只等人工 Git；之后 020 先做 zero-model admission，P0 缺口是精确模型身份、可审计 runner、输入指纹、总预算硬闸与 checkpoint，未 READY 前禁止真实调用。
...
| B11 | 009 概念层编译实现（概念主页/义项/wikilink/purpose） | **已条款化（正式 delta，Wave 2）**：`openspec/changes/009-concept-layer/`（C1–C6 + T1–T8；C3.4 混源防护；迁移 0008）；**规格复审完成（PR #12 已合入，2026-07-18），按 L4 依赖顺序可认领**；010 域段之后认领 | 开发型，中等 |
| B12 | 010 结构化直入通道实现（**双通道**：meta bootstrap→003 零 Claim / 可信业务源→Claim/QA） | ✅ **T1~T4 已合入 main（PR #14 merge `dbc073c1`，2026-07-18）**：`structured_import` 新包（通道一=003 薄编排/来源登记 fail-fast/通道二诚实门/映射合同）；codex 三轮复审收口——首轮 8/9 真伤 RED→GREEN（事务边界/迁移路径/报告诚实/歧义披露/manifest 权威常量/配置 fail-fast/防枚举/警告），二轮 mapping 顶层严格 wire model+身份归一，三轮 P0 `confirmed: StrictBool` 堵宽松布尔放宽审批门禁；**全量 1301 passed 零破坏**。**T5~T12（knowledge 域段：迁移 0007/正交 source_kind/FrozenEvidence v2/批次指纹/双轴幂等）候 021 合入后继续**——Owner-A 复审届时介入 | 开发型，中等偏大 |
| B13 | 011 知识健康度巡检实现（过期/积压/漂移/退化/孤立/同类缺口/**任务可靠性**） | **已条款化（正式 delta，Wave 2）**：`openspec/changes/011-knowledge-health/`（H1–H3 + T1–T8；迁移 0010）；**规格复审完成（PR #12 已合入，2026-07-18），按 L4 依赖顺序可认领**（实施前置：H1.3a 独立维度可并存修订已随 fast-follow 合入） | 开发型，中小 |
| B14 | 012 QA 一等对象实现（权威/派生QA，Claim绑定硬门禁） | **已条款化（正式 delta，Wave 2）**：`openspec/changes/012-qa-objects/`（Q1–Q6 + T1–T8；迁移 0009）；**规格复审完成（PR #12 已合入，2026-07-18），按 L4 依赖顺序可认领**；010 域段完成后认领（依赖 qa_staging） | 开发型，中等 |
...
| B17 | 015 数据飞轮实现（Langfuse 信号→缺口工单→回流报表） | **PR #18 codex 接管收口完成、可合并（2026-07-18）**：既有 F1 规则/构造边界脱敏/UTC 游标、F2 fail-safe 对齐+聚合、F3 报表保留；文件态持久化已推翻。迁移 `0012` 新增 Space-scoped checkpoint/processed ledger/gap 三表及复合 FK；`apply_pull` 以 KnowledgeSpace 行锁 + caller-owned transaction 实现原子 exactly-once，故障全回滚、重试不重计、同 source 跨 Space 隔离；CLI 必填 `--source-id`，dry-run零写、`--apply` 落 DB，真实 published Claim 查询激活空知识信号。新增 PostgreSQL 双会话同 trace 门禁并加入 022 精确 allowlist；本机完整 PG16 lane **4 passed / tests=4 skipped=0**，Ruff PASS、mypy 238 files PASS、deterministic **1681 passed / 9 deselected**。全仓首轮回归暴露 0012 使 `head→0002` 在 0003 拒绝前已 DDL，已按链级预检原则 RED→GREEN 修复，并拒绝 downgrade 丢弃非空飞轮表。实现 commit `f5f7f3de` 已 push；GitHub 两组 deterministic/integration-postgres/wheel-smoke 共六项全绿。继续 gated：F1.1b Langfuse 直连、F2.4 ReviewItem 动作投影、009 concept、011 合流；详见 validation-report 三版与裁决 15～17。 | 开发型，中小 |
...
| B23 | 021 Source lifecycle ordering | ✅ **T1～T10 软件完成，待人工 Git**：durable SourceHead/Event/BackfillIssue resolver、processed_at/generation、per-source lock/CAS、delete/import/notify/reactivate；迁移 0006 实际接 0012；spec PASS、quality APPROVED；deterministic 1901，PG25/skipped=0；人工提交合入后与 019 解锁 020 T1 | 开发型，中大 |
| B24 | 024 抽取召回提升（extract_empty 主攻 + 值粒度指引 + A10 弱值/兼容性护栏） | ✅ **软件实现完成（PR #13 为集成载体）**：E1～E7 机制与双侧护栏保持；新增 run-scoped `llm-attempts.sqlite`，每次出站前事务预留，hard crash 后补漏额度不复活，失败/解析重试/落选路径按 field 进入最终 pred 审计；winner/prompt/origin 从真实 producer 派生，vote 仅确证不篡改来源；assignment 独立持久化；Pipeline/实验/变体/requiredness 配置与 production/replay CLI fail-closed。当前权威设计、测试和残留边界只见 024 validation-report；**真实召回提升与同录制集 differential replay 未在本 change 证明，仍归未完成的 020 D4/D4b** | 开发型任务，中 |
