...
| B17 | 015 数据飞轮实现（Langfuse 信号→缺口工单→回流报表） | **unblocked 段实现完成（feat/015-feedback-flywheel，2026-07-17）**：`flywheel/` 新包——F1 信号提取（4 类识别器+脱敏+增量游标，`trust_env=False`）/ F2 对齐（复用 003 路由器，fail-safe 歧义不开单）+缺口聚合（稳定 ID/hit_count/reopen）/ F3 报表 + `flywheel pull` CLI 编排（默认 dry-run，`--open-tickets` 受阻非零退出）。**提交前独立红队 live 复现修 4+1 缺陷后 52 用例**；**codex PR#18 复审收口（2026-07-17）**：5 阻断+3 High 独立复核核心全部属实，spec→RED→GREEN 返工——删虚构合同 Langfuse 客户端（直连转 gated F1.1b：WeKnora 生产者根 trace 无 Q/A，须 observation 组装+citation 合同裁决）、脱敏上移 Trace 构造边界、UTC 时序+双层去重+最近样例、观察队列可消费明细、TopN 带脱敏问题+闭环周期、CLI 零副作用 dry-run/fail-closed 配置/`--apply` 文件态持久化（游标+缺口状态跨周期闭环）、正式 delta spec（strict 通过）。**69 用例（RED 22→GREEN），全量 1334 passed 零破坏，mypy 196+ruff 全绿**；validation-report 二版 + 裁决 1–14。**gated 未做**：F1.1b 直连（合同前置）、F2.4 投影+DB 三表（PR#9 已合入，剩余=subject 域协调）、009/011 候 PR#12、空知识 CLI 后端。依赖 007/003 已交付 | 开发型，中小 |
