...
> 最后更新：2026-07-21（OpenSpec 021 已随 PR #23、merge `cfefcc9b` 合入 main，六项 GitHub CI 全绿；020 T1 已更新到该基线并通过 focused 674、全仓 deterministic 2581、Ruff/mypy/OpenSpec。canonical admission 仍按设计 BLOCKED；T2～T7 未运行，只有刷新为 READY 后才允许真实模型调用；见 0j/0k/B22/B23。）
...
0h. **并行执行蓝图（总设计师制定，业务方 2026-07-16 拍板）**：全轨道结构（L0 解阻塞 / L1 关键路径 021→020 / L2 工作台 008 / L3 MCP 013 / L4 知识形态 010→009→012 / L5 抽取提准 024 / L6 治理）、优先级与护栏见 `docs/insurance-kb/22-parallel-execution-blueprint.md`；**change 号与 Alembic 迁移号一律先在 `openspec/changes/README.md` 注册表占号**（022 撞号教训；0006=021、0007=010、0008=009、0009=012 已预分配）。018/019/021/023 已完成，L1 当前进入 020 T1 PR 与真实准入解阻塞；013 与 008 W4 已随 PR #9 解锁，010 的 knowledge 域段（T5 起）也不再受 021 阻塞。**Wave 2（009/011/012）已条款化（正式 delta 过 strict；迁移 0008/0009/0010 已占号），规格复审收口中（codex P0/P1：冻结投影边界/QA 断言级绑定/三方对账等，见 PR #12）——收口前不可认领**。
...
0k. **OpenSpec 020 T1 run-admission 软件闭环（2026-07-21，已更新 021 基线）**：已按 SDD/TDD 实现类型化 admission/evaluator/CLI、签名 canonical identity、完整输入与 execution-surface 指纹、安全 provider probe、durable budget reservation/request-attempt ledger、每产品 runtime 重验、annotation/baseline 可恢复执行入口、提交前 exact-bytes/语义回验及 validation→settlement 同锁。两轮独立最终复核均无 P0/P1/P2。021 revision `f557fc94` 已固定且依赖/执行面指纹复验通过；fresh 本地 focused **674 passed**、deterministic **2581 passed/30 deselected**、Ruff/mypy 285 files/OpenSpec 全绿。权威工件仍为零模型 **`BLOCKED`**，runtime capability=`budget-ledger-v3-canary-v1`，provider probe 全为 `not_attempted`，模型调用/token/费用均为 0；剩余阻塞只有预算、11 个产品历史 provenance、一个产品缺失/未消费输入、签名审批及三模型不可变 identity。**这只表示 T1 软件闭环完成，不表示 020 数据运行完成**：T2～T7 尚未运行，D5/T8 最终对账也未完成；得到 READY 后方可按 T2→T7 执行。
...
| B11 | 009 概念层编译实现（概念主页/义项/wikilink/purpose） | **已条款化（正式 delta，Wave 2）**：`openspec/changes/009-concept-layer/`（C1–C6 + T1–T8；C3.4 混源防护；迁移 0008）；**规格复审收口中（codex P0/P1：Space 身份/定义版本化/冻结投影边界，见 PR #12）——收口前不可认领**；010 域段之后认领 | 开发型，中等 |
...
| B13 | 011 知识健康度巡检实现（过期/积压/漂移/退化/孤立/同类缺口/**任务可靠性**） | **已条款化（正式 delta，Wave 2）**：`openspec/changes/011-knowledge-health/`（H1–H3 + T1–T8；迁移 0010）；**规格复审收口中（codex P1：三方对账/typed provider/health run 持久基准/typed subject，见 PR #12）——收口前不可认领** | 开发型，中小 |
| B14 | 012 QA 一等对象实现（权威/派生QA，Claim绑定硬门禁） | **已条款化（正式 delta，Wave 2）**：`openspec/changes/012-qa-objects/`（Q1–Q6 + T1–T8；迁移 0009）；**规格复审收口中（codex P0：断言级绑定/qa_revisions/冻结投影，见 PR #12）——收口前不可认领**；010 域段完成后认领（依赖 qa_staging） | 开发型，中等 |
...
| B22 | 020 gs-v0.1 + 13 产品 baseline 真实运行 | **T1 软件闭环已更新到 021 基线并通过门禁；T2～T7 未运行**。021 依赖与执行面 identity 已通过；仍须补齐历史 provenance/缺失输入、签名审批、三模型 identity/probe 与预算准入，刷新为 READY 后才允许模型调用 | 高 token 数据任务 |
| B23 | 021 Source lifecycle ordering | ✅ **T1～T10 已随 PR #23 合入 main（merge `cfefcc9b`）**：durable SourceHead/Event/BackfillIssue resolver、processed_at/generation、per-source lock/CAS、delete/import/notify/reactivate；迁移 0006 接 0012；六项 GitHub CI 全绿 | 开发型，中大 |
...
> 016～021 已有条款级规格；验收以各 change spec、测试与 validation-report 为准。020 是环境/预算约束的数据运行，软件门禁绿不等于真实运行完成；021 ordering 能力已随 PR #23 合入。
