...
| B12 | 010 结构化直入通道实现（**双通道**：meta bootstrap→003 零 Claim / 可信业务源→Claim/QA） | 🟢 **T1~T4 波次完成（认领人=Claude 架构会话，2026-07-16，分支 `feat/010-structured-import`）**：`structured_import` 新包——通道一=003 薄编排（dry-run/apply 同路径、真实 13 产品数据集注册 100% 零 Claim）、来源登记 fail-fast、通道二诚实门（已登记也显式不可用）、映射加载/草案/manifest 四元组；19 用例 + 21 号 gauntlet 加固（构造期约束压到模型）。**codex PR #14 复审收口（2026-07-17）**：6 阻断+3 测试项独立复核，8/9 真伤 live 复现后 RED→GREEN（事务边界不越界外部 Session、CLI 绝对迁移路径、报告诚实分类版本/文档/别名副作用、草案歧义显式披露不臆断、manifest 读权威常量+注册表不可变、配置 extra=forbid+身份归一+补 record_schema_ref、防枚举空断言修复、`register`→`registration` 消 warning），阻断5"自动侦测行为变更"部分过界已反馈；**全量 1295 passed（净增 11、零破坏）、ruff/mypy 188 全绿**。**T5~T12（knowledge 域段：迁移 0007/正交 source_kind/FrozenEvidence v2/批次指纹/双轴幂等）候 018+021 合入后继续**——关键路径 018→021→020 不变；Owner-A 复审届时介入 | 开发型，中等偏大 |
