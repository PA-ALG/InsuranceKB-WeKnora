...
| B21 | 019 Golden 工具/QualityProfile/在线 Gate | ✅ **软件实施完成 + codex 三轮 review 返工**（feat/019-golden-quality-gate，T1～T6 严格 TDD，全库 non-live 1074 passed / ruff / mypy 161 files 全绿）：portable assembler+validator（强制证据回验、disputed≤5%）、**内容寻址** BaselineArtifact（六类产物各带 sha256+计数自洽）/不可变 approval（`artifact_sha256` 绑产物内容 + 强制回归 + prior 不可伪造）、QualityProfile+六维 staleness（`artifact_sha256`/`baseline_approval_sha256` 全链绑定、零观测不给满分）、`compare_baselines` 全局 micro/macro F1+幻觉+证据+unresolved+字段阈值结构化、统一 QualityGate **fail-closed**（三路径不能绕过；无 gate/画像哈希不匹配一律进 ReviewItem，不静默发布）。真实 baseline/画像由 020 用同一 API 产出。详见 `openspec/changes/019-golden-quality-gate/validation-report.md` | 开发型，中 |
