...
**新增平台独立验收仍 BLOCKED；G3 未完成。** 下方原卡历史 PASS 不覆盖用户后来要求的网页上传至发布独立验收。工作树830-g3-performance，部署源码964f968；未提交的Task3bk集中修复在当前树。复用现有服务/数据库，不新建环境。
...
当前性能/来源复用/错误及心跳诊断切片已本地验证，独立复核已通过（24文件，BLOCKER0）；自动失败文件重解析幂等接线、source失败统计、计划去重和真实4并发租约验收尚未闭合。详情：docs/insurance-kb/evidence/830-g3/task3bk-batch-diagnosis.md。下一步先收齐同批问题与修复，冻结独审后一次构建受影响组件，网页从有效候选恢复；随后仍需全新产品三材料独立验收。不得运行旧历史发布脚本。
