...
0h. **并行执行蓝图（总设计师制定，业务方 2026-07-16 拍板）**：全轨道结构（L0 解阻塞 / L1 关键路径 021→020 / L2 工作台 008 / L3 MCP 013 / L4 知识形态 010→009→012 / L5 抽取提准 024 / L6 治理）、优先级与护栏见 `docs/insurance-kb/22-parallel-execution-blueprint.md`；**change 号与 Alembic 迁移号一律先在 `openspec/changes/README.md` 注册表占号**（022 撞号教训；0006=021、0007=010、0008=009、0009=012 已预分配）。当日拍板三条：①模型凭据已补齐，交 codex 处理 023 T7/T8 → 018 T7；②013 MCP 从 M2 提前（规格就绪，实现等 PR #9 合入）；③规格工作与不触 knowledge 域的实现（008 W1–W3、010 T1~T4 通道一/登记映射、024）**不等 PR #9**，即刻从 main 开工；010 的 knowledge 域段（T5 起，structured 证据全消费链）基于 PR #9 后 main 且**先于 021 开工合入**。Wave 2（009/011/012）待架构会话完成基础对齐修订后放行。
...
| B12 | 010 结构化直入通道实现（**双通道**：meta bootstrap→003 零 Claim / 可信业务源→Claim/QA） | **已条款化（正式 delta）可认领（轨道 L4 首件，2026-07-16 三版：structured 证据全消费链合同）**：`openspec/changes/010-structured-import/`（I1–I8 + T1–T12；正交 source_kind + knowledge_id 语义 + 同键异 hash 碰撞 fail-closed + 发布链回溯 Scenario；**knowledge 域改动清单 tables/models/merge/pages+迁移 0007 全部 Owner-A 复审**；T1~T4 即刻、T5 起候 PR #9 且先于 021） | 开发型，中等偏大 |
