...
> 最后更新：2026-07-14（OpenSpec 022-review-hardening 本地实现、整包双审与终检完成；新 SHA 外部 CI 待完成）
...
0e. **OpenSpec `022-review-hardening` 本地实现、整包双审与终检完成，当前仍为未提交 working tree**：六项 Claude 复审意见已客观裁决并按 SDD/TDD 实现；RH1～RH6 task-level 双审均 Approved，整包 spec review `Approved`，整包 quality 的唯一 Important（持久化 `proposed=[]` 泄漏 `AttributeError`）已补 RED、在 RH2 边界统一为无泄漏 `ScopeViolation` 后复审 `Approved`，无剩余 Critical/Important/Minor。最终 fresh 本地证据：OpenSpec strict exit 0、Ruff exit 0、mypy 151 files exit 0、deterministic **961 passed / 5 deselected**；focused 为 RH1 `43 passed / 1 skipped`、RH2 `62 passed`、RH3 `136 passed`、RH4 `110 passed`、RH5/RH6 `15 passed`，bridge collection 仍为 contract 12 / live 1。**尚未完成的是人类验收 commit/push 与最终 SHA 的 PostgreSQL 16 CI**；所以 integration 状态为 **`PENDING HUMAN PUSH/CI`**，WeKnora 仍为 **`NOT RUN`**，不得借用 0d 的旧 commit/job 覆盖本 working tree，也不得声称 PR 已更新。RH1 只保证函数内 savepoint，outer transaction/进程终止/多页补偿仍归 018；RH2 不含 ordering/`processed_at`/SourceHead/CAS，仍归 021；Directory replay 保持 eval-only；live 是 existing-knowledge 而非 upload。下一步：人类复核 working tree 后 commit/push，等待最终 SHA 的 PostgreSQL 16 CI；`harness/README.md` 的旧 deterministic 命令另作非本 change 文档 follow-up。
