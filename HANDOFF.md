...
| B24 | 024 抽取召回提升（extract_empty 主攻 + 值粒度指引 + A10 弱值/兼容性护栏） | 🟡 **PR #13 接管收口中**：既有 E1～E7 机制、审计与护栏已实现；codex 正在修复 durable 调用预算/attempt 事实账本、权威文档一致性与 fail-closed 运行配置，完成全量门禁后合入 main；真实召回与 differential replay 仍归 020 D4b | 开发型任务，中 |
