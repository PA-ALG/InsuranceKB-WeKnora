...
| B24 | 024 抽取召回提升（extract_empty 主攻 + 值粒度指引 + A10 弱值/兼容性护栏） | 🟡 **T1~T7 软件完成 + codex PR#13 六项发现返工闭合（2026-07-18，rebase main@dbc073c1）**：①pred 携带类型化 extraction_audit 过 pred.jsonl 交付边界；②实验三概念分离（确定性分桶/实际所用模板/胜出路径），注册表+策略注入且内容摘要入 run/checkpoint 身份（resume 不一致 fail-closed）；③E3 触发合同落地（requiredness+预算原子扣减+无候选零调用）；④source_pointer 词条参与定向检索；⑤E5 探针诚实化为机制合同、非退化让渡 020 D4；⑥错误语义断言替换。门禁：ruff/mypy 226 files/deterministic 1668 passed/openspec strict。**待 codex 复审，未合 main** | 开发型任务，中 |
