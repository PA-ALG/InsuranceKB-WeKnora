...
> 最后更新：2026-07-18（PR #18 由 codex 接管：文件态已替换为 migration 0012 的 Space-scoped durable unit-of-work；本机 PostgreSQL 16 四节点 `tests=4 skipped=0`、Ruff/mypy 全绿、deterministic `1681 passed / 9 deselected`，尚未 push 新 SHA，见 0i/B17。018 已随 PR #9 合入 `main`；008 工作台 PR #15 仍待终审，见 B8。）
...
| B17 | 015 数据飞轮实现（Langfuse 信号→缺口工单→回流报表） | **PR #18 codex 接管、本地收口完成（2026-07-18）**：既有 F1 规则/构造边界脱敏/UTC 游标、F2 fail-safe 对齐+聚合、F3 报表保留；文件态持久化已推翻。迁移 `0012` 新增 Space-scoped checkpoint/processed ledger/gap 三表及复合 FK；`apply_pull` 以 KnowledgeSpace 行锁 + caller-owned transaction 实现原子 exactly-once，故障全回滚、重试不重计、同 source 跨 Space 隔离；CLI 必填 `--source-id`，dry-run零写、`--apply` 落 DB，真实 published Claim 查询激活空知识信号。新增 PostgreSQL 双会话同 trace 门禁并加入 022 精确 allowlist；本机完整 PG16 lane **4 passed / tests=4 skipped=0**，Ruff PASS、mypy 238 files PASS、deterministic **1681 passed / 9 deselected**。全仓首轮回归暴露 0012 使 `head→0002` 在 0003 拒绝前已 DDL，已按链级预检原则 RED→GREEN 修复，并拒绝 downgrade 丢弃非空飞轮表。**待 commit/push、新 SHA GitHub 两条 CI 后方可合并**。继续 gated：F1.1b Langfuse 直连、F2.4 ReviewItem 动作投影、009 concept、011 合流；详见 validation-report 三版与裁决 15～16。 | 开发型，中小 |
