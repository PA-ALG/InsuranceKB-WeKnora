...
| B24 | 024 抽取召回提升（extract_empty 主攻 + 值粒度指引 + A10 弱值/兼容性护栏） | ✅ **T1~T7 软件完成（认领人=Claude 架构会话，2026-07-16，分支 `feat/024-extraction-recall-uplift`）**：38 条 RED→GREEN + E5 探针 11 用例（三重钉桩）；变体机制（default@v1/targeted@v1）+ 定向短答模板（16 工单字段）+ 粒度指引（10 字段）+ E6 两族清洗与 Q012 兼容性护栏（校验链 2.5 步）；**全量 deterministic 1314 passed（基线 1265 零破坏）、ruff/mypy 185 files 全绿**；真实召回结论显式留待 020 D4（变体版本对账）。详见 024/validation-report.md；**待 PR 双查（Owner=B 域）** | 开发型，中小 |
